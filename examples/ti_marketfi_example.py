...
def main():
    ...
    simd_inputs = [
        [asset1_high_vec, asset1_low_vec, asset1_volume_vec],  # Asset 1
        [asset2_high_vec, asset2_low_vec, asset2_volume_vec],  # Asset 2
        [asset3_high_vec, asset3_low_vec, asset3_volume_vec],  # Asset 3
        [asset4_high_vec, asset4_low_vec, asset4_volume_vec],  # Asset 4
    ]
    ...
    try:
        # Calculate MARKETFI for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.marketfi.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MARKETFI values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.marketfi.indicator(

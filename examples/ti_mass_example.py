#!/usr/bin/env python3
"""
Python example for the MASS indicator from tulip_rs_python.
...
def main():
    # Sample data from Rust example - base data extended
    high_base = [
        82.15,
        81.89,
        83.03,
    ...
    # Extend the data as in Rust example
    high = high_base + high_base
    low = low_base + low_base
    ...
    info = tulip_rs.indicators.mass.info()
    ...
    min_data = tulip_rs.indicators.mass.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.mass.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full MASS Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.mass.indicator(inputs, options, optional_outputs)

    print(f"Full Mass Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial MASS Line
    ...
    outputs2, state2 = tulip_rs.indicators.mass.indicator(inputs2, options)
    ...
    try:
        ...
        print("State info: MASS State - internal state for Mass Index")
    ...
    print(f"\nFinal Mass Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.mass.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Mass Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        # Calculate MASS for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.mass.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MASS values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.mass.indicator(
    ...
    try:
        # Calculate MASS for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.mass.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} MASS values (first 5): {output[0][:5]}")

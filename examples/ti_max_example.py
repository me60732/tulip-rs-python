#!/usr/bin/env python3
"""
Python example for the MAX indicator from tulip_rs_python.
...
def main():
    ...
    info = tulip_rs.indicators.max.info()
    ...
    min_data = tulip_rs.indicators.max.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.max.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full MAX Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.max.indicator(inputs, options, optional_outputs)

    print(f"Full max Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial MAX Line
    ...
    outputs2, state2 = tulip_rs.indicators.max.indicator(inputs2, options)
    print(f"\nmax Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: MAX State - internal state for Maximum Value")
    ...
    print(f"New max Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.max.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification max Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        # Calculate MAX for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.max.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MAX values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.max.indicator(
    ...
    try:
        # Calculate MAX for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.max.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} MAX values (first 5): {output[0][:5]}")

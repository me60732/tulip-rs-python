#!/usr/bin/env python3
"""
Python example for the CHANDELIEREXIT indicator from tulip_rs_python.

This example demonstrates:
1. Basic Chandelier Exit calculation with optional outputs
...
def main():
    ...
    # Options for Chandelier Exit: period=5, multiplier=3.0 (matches Rust reference example)
    options = [5.0, 3.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.chandelierexit.info()
    ...
    min_data = tulip_rs.indicators.chandelierexit.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.chandelierexit.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.chandelierexit.indicator(
        inputs, options, optional_outputs
    )

    print("Full dataset calculation:")
    print(f"Chandelier Exit Long: {outputs[0]}")
    print(f"Chandelier Exit Short: {outputs[1]}")
    ...
    outputs_partial, state = tulip_rs.indicators.chandelierexit.indicator(
        inputs_partial, options
    )

    print(f"\nPartial calculation (first {len(partial_close)} elements):")
    print(f"Chandelier Exit Long: {outputs_partial[0]}")
    print(f"Chandelier Exit Short: {outputs_partial[1]}")
    ...
    print(f"New Chandelier Exit Long: {continued_outputs[0]}")
    print(f"New Chandelier Exit Short: {continued_outputs[1]}")
    ...
    try:
        # Calculate Chandelier Exit for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.chandelierexit.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Chandelier Exit Long: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.chandelierexit.indicator(
                asset_inputs, options, optional_outputs
            )
            print(f"Asset {i + 1} individual Long: {individual_output[0]}")
    ...
    simd_options = [
        [5.0, 3.0],  # Option set 1 (matches Rust reference)
        [7.0, 2.0],  # Option set 2
        [10.0, 3.0],  # Option set 3
    ...
    try:
        # Calculate Chandelier Exit for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = (
            tulip_rs.indicators.chandelierexit.simd_by_options(
        ...
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} Chandelier Exit Long (first 5): {output[0][:5]}")
        ...
        for i, opt in enumerate(simd_options):
            ...
            print(
                f"Option set {i + 1} individual Long (first 5): {individual_output[0][:5]}"
            )

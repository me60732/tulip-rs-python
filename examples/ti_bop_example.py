#!/usr/bin/env python3
"""
Python example for the BOP indicator from tulip_rs_python.

This example demonstrates:
1. Basic BOP calculation with optional outputs
...
def main():
    ...
    # Options for BOP: none (matching Rust example)
    options = []

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.bop.info()
    ...
    min_data = tulip_rs.indicators.bop.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.bop.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.bop.indicator(inputs, options, optional_outputs)

    print("Full dataset calculation:")
    print(f"BOP Line: {outputs[0]}")
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.bop.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} BOP values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.bop.indicator(

#!/usr/bin/env python3
"""
Python example for the MEDPRICE indicator from tulip_rs_python.

This example demonstrates:
1. Basic MEDPRICE calculation
...
def main():
    ...
    info = tulip_rs.indicators.medprice.info()
    ...
    min_data = tulip_rs.indicators.medprice.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.medprice.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full MEDPRICE Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.medprice.indicator(
        inputs, options, optional_outputs
    )

    print(f"Full MEDPRICE Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial MEDPRICE Line
    ...
    outputs2, state2 = tulip_rs.indicators.medprice.indicator(inputs2, options)
    print(f"MEDPRICE Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: MEDPRICE State - internal state for Median Price")
    ...
    print(f"\nNew MEDPRICE Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.medprice.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification MEDPRICE Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        # Calculate MEDPRICE for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.medprice.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MEDPRICE values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.medprice.indicator(

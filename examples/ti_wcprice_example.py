#!/usr/bin/env python3
"""
Python example for the WCPRICE indicator from tulip_rs_python.

This example demonstrates:
1. Basic WCPRICE calculation
...
def main():
    ...
    info = tulip_rs.indicators.wcprice.info()
    ...
    min_data = tulip_rs.indicators.wcprice.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.wcprice.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full WCPRICE Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.wcprice.indicator(
        inputs, options, optional_outputs
    )

    print(f"WCPrice Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial WCPRICE Line
    ...
    outputs2, state2 = tulip_rs.indicators.wcprice.indicator(inputs2, options)
    print(f"Partial WCPrice Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: WCPRICE State - internal state for Weighted Close Price")
    ...
    print(f"Continued WCPrice Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.wcprice.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification WCPrice Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.wcprice.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} WCPRICE values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.wcprice.indicator(

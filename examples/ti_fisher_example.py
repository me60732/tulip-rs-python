#!/usr/bin/env python3
"""
Python example for the FISHER indicator from tulip_rs_python.

This example demonstrates:
1. Basic FISHER calculation with optional outputs
...
def main():
    ...
    # Options for FISHER: period=5 (matching Rust example)
    options = [5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.fisher.info()
    ...
    min_data = tulip_rs.indicators.fisher.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.fisher.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.fisher.indicator(inputs, options, optional_outputs)

    print("Full dataset calculation:")
    print(f"Full Fisher Transform Line: {outputs[0]}")
    print(f"Full Fisher Signal Line: {outputs[1]}")
    ...
    outputs_partial, state = tulip_rs.indicators.fisher.indicator(
        inputs_partial, options
    )

    print(f"\nPartial calculation (first {len(partial_high)} elements):")
    print(f"Partial Fisher Transform Line: {outputs_partial[0]}")
    print(f"Partial Fisher Signal Line: {outputs_partial[1]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")

    # Get state info
    print(f"State info: Fisher State - internal state for Fisher Transform")
    ...
    print(f"Final Fisher Transform Line: {continued_outputs[0]}")
    print(f"Final Fisher Signal Line: {continued_outputs[1]}")
    ...
    try:
        # Calculate FISHER for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.fisher.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} FISHER values (first 5): {output[0][:5]}")

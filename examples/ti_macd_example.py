...
def main():
    ...
    simd_options = [
        [1.0, 3.0, 5.0],  # Option set 1
        [2.0, 5.0, 9.0],  # Option set 2 (Original)
        [3.0, 8.0, 14.0],  # Option set 3
        [4.0, 10.0, 18.0],  # Option set 4
    ]
    ...
    try:
        # Calculate MACD for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.macd.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} MACD values (first 5): {output[0][:5]}")

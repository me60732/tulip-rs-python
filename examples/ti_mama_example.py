#!/usr/bin/env python3
"""
Python example for the MAMA (MESA Adaptive Moving Average) indicator from tulip_rs_python.

This example demonstrates:
1. Basic mama calculation with optional outputs
...
def main():
    ...
    inputs = [close]
    options = [0.5, 0.05]  # fast_limit, slow_limit

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.mama.info()
    ...
    min_data = tulip_rs.indicators.mama.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full MAMA
    optional_outputs = [True] * len(info["optional_outputs"])
    outputs, _ = tulip_rs.indicators.mama.indicator(inputs, options, optional_outputs)

    print(f"FULL MAMA: {outputs[0]}")
    print(f"FULL FAMA: {outputs[1]}")
    print(f"FULL DC Period (optional): {outputs[2]}")
    print(f"FULL Alpha (optional): {outputs[3]}")

    ################################################### Calculating the Partial MAMA
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.mama.indicator(inputs_partial, options)
    print(f"\nMAMA: {outputs_partial[0]}")
    print(f"FAMA: {outputs_partial[1]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew MAMA: {continued_outputs[0]}")
    print(f"New FAMA: {continued_outputs[1]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.mama.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification MAMA: {full_outputs[0]}")
    print(f"Verification FAMA: {full_outputs[1]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.mama.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MAMA values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.mama.indicator(
    ...
    simd_options = [
        [0.5, 0.05],  # Option set 1: fast=0.5, slow=0.05 (original)
        [0.4, 0.04],  # Option set 2: fast=0.4, slow=0.04
        [0.6, 0.06],  # Option set 3: fast=0.6, slow=0.06
        [0.7, 0.07],  # Option set 4: fast=0.7, slow=0.07
    ]

    print(f"Processing {len(simd_options)} option sets simultaneously using SIMD...")
    for i, opt in enumerate(simd_options):
        print(f"Option set {i + 1}: fast_limit={opt[0]}, slow_limit={opt[1]}")
    print()

    try:
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.mama.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} MAMA values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the NVI indicator from tulip_rs_python.

This example demonstrates:
1. Basic NVI calculation
...
def main():
    ...
    info = tulip_rs.indicators.nvi.info()
    ...
    min_data = tulip_rs.indicators.nvi.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.nvi.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full NVI Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.nvi.indicator(inputs, options, optional_outputs)

    print(f"Full NVI Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial NVI Line
    ...
    outputs2, state2 = tulip_rs.indicators.nvi.indicator(inputs2, options)
    print(f"NVI Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: NVI State - internal state for Negative Volume Index")
    ...
    full_outputs, _ = tulip_rs.indicators.nvi.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification NVI Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.nvi.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} NVI values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.nvi.indicator(

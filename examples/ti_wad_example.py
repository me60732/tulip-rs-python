#!/usr/bin/env python3
"""
Python example for the WAD indicator from tulip_rs_python.

This example demonstrates:
1. Basic WAD calculation
...
def main():
    ...
    info = tulip_rs.indicators.wad.info()
    ...
    min_data = tulip_rs.indicators.wad.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.wad.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full WAD Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.wad.indicator(inputs, options, optional_outputs)

    print(f"Full WAD Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial WAD Line
    ...
    outputs2, state2 = tulip_rs.indicators.wad.indicator(inputs2, options)
    print(f"Partial WAD Line: {outputs2[0]}")
    ...
    print(f"New WAD Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.wad.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification WAD Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.wad.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} WAD values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.wad.indicator(

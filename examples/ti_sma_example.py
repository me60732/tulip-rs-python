#!/usr/bin/env python3
"""
Python example for the SMA indicator from tulip_rs_python.
...
def main():
    ...
    min_data = tulip_rs.indicators.sma.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.sma.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.sma.indicator(inputs, options, optional_outputs)

    print(f"Full SMA Line: {outputs[0]}")
    if optional_count > 0:
    ...
    outputs2, state2 = tulip_rs.indicators.sma.indicator(inputs2, options)
    print(f"Partial SMA Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: SMA State - internal state for Simple Moving Average")
    ...
    print(f"Final SMA Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.sma.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification SMA Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.sma.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} SMA values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.sma.indicator(
    ...
    try:
        # Calculate SMA for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.sma.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} SMA values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the EMA indicator from tulip_rs_python.
...
def main():
    ...
    # Show indicator info
    info = tulip_rs.indicators.ema.info()
    ...
    min_data = tulip_rs.indicators.ema.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.ema.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    # Full calculation - matches Rust example
    outputs, _ = tulip_rs.indicators.ema.indicator(inputs, options)
    print(f"Full EMA Line: {outputs[0]}")
    ...
    inputs2 = [partial_close_vec]

    outputs2, state = tulip_rs.indicators.ema.indicator(inputs2, options)
    ...
    final_outputs = state.batch_indicator(new_inputs)
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.ema.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} EMA values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.ema.indicator(
    ...
    try:
        # Calculate EMA for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.ema.simd_by_options(
            expanded_inputs, simd_options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} EMA values (first 5): {output[0][:5]}")

...
def main():
    ...
    try:
        # Calculate BBANDS for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.bbands.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} BBANDS values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.bbands.indicator(
    ...
    simd_options = [
        [3.0, 1.0],  # Option set 1
        [5.0, 2.0],  # Option set 2 (Original)
        [7.0, 3.0],  # Option set 3
        [10.0, 4.0],  # Option set 4
    ]
    ...
    try:
        # Calculate BBANDS for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.bbands.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} BBANDS values (first 5): {output[0][:5]}")

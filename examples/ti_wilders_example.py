#!/usr/bin/env python3
"""
Python example for the WILDERS indicator from tulip_rs_python.

This example demonstrates:
1. Basic WILDERS calculation
...
def main():
    ...
    info = tulip_rs.indicators.wilders.info()
    ...
    min_data = tulip_rs.indicators.wilders.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.wilders.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full WILDERS Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.wilders.indicator(
        inputs, options, optional_outputs
    )

    print(f"Full Wilders Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial WILDERS Line
    ...
    outputs2, state2 = tulip_rs.indicators.wilders.indicator(inputs2, options)
    print(f"Wilders Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: WILDERS State - internal state for Wilders Smoothing")
    ...
    print(f"\nNew Wilders Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.wilders.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Wilders Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.wilders.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} WILDERS values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.wilders.indicator(
    ...
    try:
        # Calculate WILDERS for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.wilders.simd_by_options(
            expanded_inputs, simd_options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} WILDERS values (first 5): {output[0][:5]}")

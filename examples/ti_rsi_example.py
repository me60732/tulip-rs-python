#!/usr/bin/env python3
"""
Python example for the RSI indicator from tulip_rs_python.

This example demonstrates:
1. Basic RSI calculation with optional outputs
...
def main():
    ...
    info = tulip_rs.indicators.rsi.info()
    ...
    min_data = tulip_rs.indicators.rsi.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.rsi.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full RSI Line with Optional Outputs
    ...
    outputs, _ = tulip_rs.indicators.rsi.indicator(inputs, options, optional_outputs)

    print(f"Full RSI Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial RSI Line
    ...
    outputs2, state2 = tulip_rs.indicators.rsi.indicator(
        inputs2, options, optional_outputs
    )
    print(f"RSI Line: {outputs2[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        print("State info: RSI State - internal state for Relative Strength Index")
    ...
    # State continuation - specific pattern for RSI (matching Rust example)
    continuation_optional = [False, False, False] if optional_count > 0 else None
    final_outputs = state2.batch_indicator(new_data, continuation_optional)
    print(f"New RSI Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.rsi.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification RSI Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.rsi.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} RSI values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.rsi.indicator(
    ...
    try:
        # Calculate RSI for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.rsi.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} RSI values (first 5): {output[0][:5]}")

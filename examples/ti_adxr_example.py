#!/usr/bin/env python3
"""
Python example for the ADXR indicator from tulip_rs_python.

This example demonstrates:
1. Basic ADXR calculation
...
def main():
    ...
    info = tulip_rs.indicators.adxr.info()
    ...
    min_data = tulip_rs.indicators.adxr.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.adxr.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full ADXR Line with Optional Outputs
    ...
    outputs, _ = tulip_rs.indicators.adxr.indicator(inputs, options, optional_outputs)

    print(f"Full ADXR Line: {outputs[0]}")
    if optional_count > 0:
        print(f"Full ADX Line: {outputs[1]}")
        print(f"Full DX Line: {outputs[2]}")
        print(f"Full ATR Line: {outputs[3]}")
        print(f"Full TR Line: {outputs[4]}")

    ################################################### Calculating the partial ADXR Line
    # Use partial data for state demo
    high_vec2 = np.array(high[:-2], dtype=np.float64)
    low_vec2 = np.array(low[:-2], dtype=np.float64)
    close_vec2 = np.array(close[:-2], dtype=np.float64)
    inputs2 = [high_vec2, low_vec2, close_vec2]

    # Partial calculation - also with optional outputs (different from previous pattern)
    outputs2, state2 = tulip_rs.indicators.adxr.indicator(
        inputs2, options, optional_outputs
    )
    print(f"\n\nPartial ADXR Line: {outputs2[0]}")
    if optional_count > 0:
        print(f"Partial ADX Line: {outputs2[1]}")
        print(f"Partial DX Line: {outputs2[2]}")
        print(f"Partial ATR Line: {outputs2[3]}")
        print(f"Partial TR Line: {outputs2[4]}")
    ...
    new_high_vec = np.array(high[-2:], dtype=np.float64)
    new_low_vec = np.array(low[-2:], dtype=np.float64)
    new_close_vec = np.array(close[-2:], dtype=np.float64)
    ...
    print(f"\n\nFinal ADXR Line: {final_outputs[0]}")
    if len(final_outputs) > 1:
        print(f"Final ADX Line: {final_outputs[1]}")
        print(f"Final DX Line: {final_outputs[2]}")
        print(f"Final ATR Line: {final_outputs[3]}")
        print(f"Final TR Line: {final_outputs[4]}")
    ...
    full_outputs, _ = tulip_rs.indicators.adxr.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification ADXR Line: {full_outputs[0]}")
    if optional_count > 0:
        print(f"Verification ADX Line: {full_outputs[1]}")
        print(f"Verification DX Line: {full_outputs[2]}")
        print(f"Verification ATR Line: {full_outputs[3]}")
        print(f"Verification TR Line: {full_outputs[4]}")
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.adxr.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} ADXR values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.adxr.indicator(
    ...
    try:
        # Calculate ADXR for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.adxr.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} ADXR values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the EF (Efficiency Ratio) indicator from tulip_rs_python.

This example demonstrates:
1. Basic Efficiency Ratio calculation with optional outputs
...
def main():
    ...
    # Options for Efficiency Ratio: period=5 (matches Rust reference example)
    options = [5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.ef.info()
    ...
    min_data = tulip_rs.indicators.ef.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.ef.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.ef.indicator(inputs, options, optional_outputs)

    print("Full dataset calculation:")
    print(f"Efficiency Ratio: {outputs[0]}")
    ...
    outputs_partial, state = tulip_rs.indicators.ef.indicator(inputs_partial, options)

    print(f"\nPartial calculation (first {len(partial_close)} elements):")
    print(f"Efficiency Ratio: {outputs_partial[0]}")
    ...
    print(f"New Efficiency Ratio: {continued_outputs[0]}")
    ...
    try:
        # Calculate Efficiency Ratio for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.ef.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Efficiency Ratio: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.ef.indicator(
    ...
    simd_options = [
        [3.0],  # Option set 1
        [5.0],  # Option set 2 (matches Rust reference)
        [10.0],  # Option set 3
        [14.0],  # Option set 4
    ]
    ...
    try:
        # Calculate Efficiency Ratio for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.ef.simd_by_options(
            inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} Efficiency Ratio (first 5): {output[0][:5]}")

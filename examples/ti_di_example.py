#!/usr/bin/env python3
"""
Python example for the DI indicator from tulip_rs_python.

This example demonstrates:
1. Basic DI calculation with optional outputs
...
def main():
    ...
    # Options for DI: period=5 (matching Rust example)
    options = [5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.di.info()
    ...
    min_data = tulip_rs.indicators.di.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.di.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.di.indicator(inputs, options, [True, True])

    print("Full dataset calculation:")
    print(f"Full Plus DI: {outputs[0]}")
    print(f"\n\nFull Minus DI: {outputs[1]}")
    print(f"atr: {outputs[2]}")
    print(f"tr: {outputs[3]}")
    ...
    outputs_partial, state = tulip_rs.indicators.di.indicator(inputs_partial, options)

    print(f"\nPartial calculation (first {len(partial_high)} elements):")
    print(f"Partial Plus DI: {outputs_partial[0]}")
    print(f"Partial Minus DI: {outputs_partial[1]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")

    # Get state info
    print(f"State info: DI State - internal state for Directional Indicator")
    ...
    print(f"Final Plus DI: {continued_outputs[0]}")
    print(f"Final Minus DI: {continued_outputs[1]}")
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.di.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} DI values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.di.indicator(
    ...
    try:
        # Calculate DI for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.di.simd_by_options(
            expanded_inputs, simd_options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} DI values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the Homodyne Discriminator indicator from tulip_rs_python.

This example demonstrates:
1. Basic homodynediscriminator calculation (no optional outputs)
...
def main():
    ...
    info = tulip_rs.indicators.homodynediscriminator.info()
    ...
    min_data = tulip_rs.indicators.homodynediscriminator.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Homodyne Discriminator
    # No optional outputs for this indicator
    optional_outputs = None
    outputs, _ = tulip_rs.indicators.homodynediscriminator.indicator(
        inputs, options, optional_outputs
    )

    print(f"FULL DC Period: {outputs[0]}")

    ################################################### Calculating the Partial Homodyne Discriminator
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.homodynediscriminator.indicator(
        inputs_partial, options
    )
    print(f"\nDC Period: {outputs_partial[0]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew DC Period: {continued_outputs[0]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.homodynediscriminator.indicator(
        inputs, options
    )
    print(f"Verification DC Period: {full_outputs[0]}")
    ...
    try:
        simd_outputs, simd_states = (
            tulip_rs.indicators.homodynediscriminator.simd_by_assets(
                simd_inputs, options
            )
        )
        ...
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} DC Period values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.homodynediscriminator.indicator(

...
def main():
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.cci.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} CCI values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.cci.indicator(
    ...
    try:
        # Calculate CCI for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.cci.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} CCI values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the Vortex indicator from tulip_rs_python.

This example demonstrates:
1. Basic Vortex calculation with optional outputs
...
def main():
    # Test Input Data - 20 bars (no volume for Vortex)
    high = [
        82.15,
        81.89,
        83.03,
    ...
    # Show indicator info
    info = tulip_rs.indicators.vortex.info()
    ...
    min_data = tulip_rs.indicators.vortex.min_data(options)
    ...
    outputs, _ = tulip_rs.indicators.vortex.indicator(inputs, options, optional_outputs)
    print(f"Full VI+ (vi_up) Line:  {outputs[0]}")
    print(f"Full VI- (vi_down) Line: {outputs[1]}")
    if optional_outputs and len(outputs) > 2:
        print(f"Full True Range (tr) Line: {outputs[2]}")
    ...
    outputs2, state = tulip_rs.indicators.vortex.indicator(
        inputs2, options, optional_outputs
    )
    print(f"\nVI+ Line (15 bars): {outputs2[0]}")
    print(f"VI- Line (15 bars): {outputs2[1]}")
    ...
    print(f"\nNew VI+ Line (5 bars): {new_outputs[0]}")
    print(f"New VI- Line (5 bars): {new_outputs[1]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.vortex.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} VI+ values: {output[0]}")
            print(f"Asset {i + 1} VI- values: {output[1]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.vortex.indicator(
                asset_inputs, options, optional_outputs
            )
            print(f"Asset {i + 1} individual VI+: {individual_output[0]}")

            if np.allclose(
                simd_outputs[i][0], individual_output[0], rtol=1e-10, equal_nan=True
            ):
                print(f"✓ Asset {i + 1} SIMD VI+ matches individual calculation")
            else:
                print(f"✗ Asset {i + 1} SIMD VI+ does not match individual calculation")
    ...
    try:
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.vortex.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} VI+ values (first 5): {output[0][:5]}")
            print(f"Option set {i + 1} VI- values (first 5): {output[1][:5]}")
        ...
        for i, opt in enumerate(simd_options):
            ...
            print(
                f"Option set {i + 1} individual VI+ (first 5): {individual_output[0][:5]}"
            )

...
def main():
    ...
    try:
        # Calculate CVI for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.cvi.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} CVI values (first 5): {output[0][:5]}")

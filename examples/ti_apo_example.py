#!/usr/bin/env python3
"""
Python example for the APO indicator from tulip_rs_python.

This example demonstrates:
1. Basic APO calculation with optional outputs
...
def main():
    ...
    # Options for APO: short_period=2, long_period=5 (matching Rust example)
    options = [2.0, 5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.apo.info()
    ...
    min_data = tulip_rs.indicators.apo.min_data(options)
    ...
    outputs, _ = tulip_rs.indicators.apo.indicator(inputs, options, [True, True])

    print("Full dataset calculation:")
    print(f"Full APO Line: {outputs[0]}")
    print(f"Short EMA Line: {outputs[1]}")
    print(f"Long EMA Line: {outputs[2]}")
    ...
    outputs_partial, state = tulip_rs.indicators.apo.indicator(inputs_partial, options)

    print(f"\nPartial calculation (first {len(partial_close)} elements):")
    print(f"APO Line: {outputs_partial[0]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")

    # Get state info
    print(f"State info: APO State - internal state for Absolute Price Oscillator")
    ...
    print(f"New APO Line: {continued_outputs[0]}")
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.apo.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} APO values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.apo.indicator(
    ...
    try:
        # Calculate APO for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.apo.simd_by_options(
            expanded_inputs, simd_options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} APO values (first 5): {output[0][:5]}")

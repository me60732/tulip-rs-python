#!/usr/bin/env python3
"""
Python example for the Hilbert Transform indicator from tulip_rs_python.

This example demonstrates:
1. Basic hilberttransform calculation with optional outputs
...
def main():
    ...
    inputs = [close]
    options = [10.0, 20.0]  # ss_period, hp_period

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.hilberttransform.info()
    ...
    min_data = tulip_rs.indicators.hilberttransform.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Hilbert Transform
    optional_outputs = [True] * len(info["optional_outputs"])
    outputs, _ = tulip_rs.indicators.hilberttransform.indicator(
        inputs, options, optional_outputs
    )

    print(f"FULL In-Phase: {outputs[0]}")
    print(f"FULL Quadrature: {outputs[1]}")
    print(f"FULL Roofing (optional): {outputs[2]}")
    print(f"FULL Highpass (optional): {outputs[3]}")

    ################################################### Calculating the Partial Hilbert Transform
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.hilberttransform.indicator(
        inputs_partial, options
    )
    print(f"\nIn-Phase: {outputs_partial[0]}")
    print(f"Quadrature: {outputs_partial[1]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew In-Phase: {continued_outputs[0]}")
    print(f"New Quadrature: {continued_outputs[1]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.hilberttransform.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification In-Phase: {full_outputs[0]}")
    print(f"Verification Quadrature: {full_outputs[1]}")
    print(f"Verification Roofing (optional): {full_outputs[2]}")
    print(f"Verification Highpass (optional): {full_outputs[3]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.hilberttransform.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} In-Phase values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.hilberttransform.indicator(
    ...
    simd_options = [
        [5.0, 10.0],  # Option set 1: ss_period=5, hp_period=10
        [10.0, 20.0],  # Option set 2: ss_period=10, hp_period=20 (original)
        [15.0, 30.0],  # Option set 3: ss_period=15, hp_period=30
        [20.0, 40.0],  # Option set 4: ss_period=20, hp_period=40
    ]

    print(f"Processing {len(simd_options)} option sets simultaneously using SIMD...")
    for i, opt in enumerate(simd_options):
        print(f"Option set {i + 1}: ss_period={opt[0]}, hp_period={opt[1]}")
    print()

    try:
        simd_opt_outputs, simd_opt_states = (
            tulip_rs.indicators.hilberttransform.simd_by_options(
        ...
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} In-Phase values (first 5): {output[0][:5]}")

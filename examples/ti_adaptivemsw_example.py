#!/usr/bin/env python3
"""
Python example for the Adaptive MSW (Adaptive Mesa Sine Wave) indicator from tulip_rs_python.

This example demonstrates:
1. Basic adaptivemsw calculation with optional outputs
...
def main():
    ...
    info = tulip_rs.indicators.adaptivemsw.info()
    ...
    min_data = tulip_rs.indicators.adaptivemsw.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Adaptive MSW
    optional_outputs = [True] * len(info["optional_outputs"])
    outputs, _ = tulip_rs.indicators.adaptivemsw.indicator(
        inputs, options, optional_outputs
    )

    print(f"FULL Adaptive MSW Sine: {outputs[0]}")
    print(f"FULL Adaptive MSW Lead Sine: {outputs[1]}")
    print(f"FULL DC Period (optional): {outputs[2]}")

    ################################################### Calculating the Partial Adaptive MSW
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.adaptivemsw.indicator(
        inputs_partial, options
    )
    print(f"\nAdaptive MSW Sine: {outputs_partial[0]}")
    print(f"Adaptive MSW Lead Sine: {outputs_partial[1]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew Adaptive MSW Sine: {continued_outputs[0]}")
    print(f"New Adaptive MSW Lead Sine: {continued_outputs[1]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.adaptivemsw.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Adaptive MSW Sine: {full_outputs[0]}")
    print(f"Verification Adaptive MSW Lead Sine: {full_outputs[1]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.adaptivemsw.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Sine values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.adaptivemsw.indicator(

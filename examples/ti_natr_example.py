#!/usr/bin/env python3
"""
Python example for the NATR indicator from tulip_rs_python.

This example demonstrates:
1. Basic NATR calculation
...
def main():
    ...
    info = tulip_rs.indicators.natr.info()
    ...
    min_data = tulip_rs.indicators.natr.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.natr.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full NATR Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.natr.indicator(inputs, options, optional_outputs)

    print(f"Full NATR Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial NATR Line
    # Use partial data for state demo (remove last 1 point, matching Rust example)
    high_vec2 = np.array(high[:-1], dtype=np.float64)
    low_vec2 = np.array(low[:-1], dtype=np.float64)
    ...
    outputs2, state2 = tulip_rs.indicators.natr.indicator(inputs2, options)
    print(f"NATR Line: {outputs2[0]}")
    ...
    try:
        ...
        print(
            "State info: NATR State - internal state for Normalized Average True Range"
        )
    ...
    new_high_vec = np.array(high[-1:], dtype=np.float64)
    new_low_vec = np.array(low[-1:], dtype=np.float64)
    ...
    full_outputs, _ = tulip_rs.indicators.natr.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification NATR Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        # Calculate NATR for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.natr.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} NATR values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.natr.indicator(
    ...
    try:
        # Calculate NATR for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.natr.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} NATR values (first 5): {output[0][:5]}")

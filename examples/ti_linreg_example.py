#!/usr/bin/env python3
"""
Python example for the LINREG indicator from tulip_rs_python.
...
def main():
    ...
    info = tulip_rs.indicators.linreg.info()
    ...
    min_data = tulip_rs.indicators.linreg.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.linreg.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full LINREG Line with Optional Outputs
    ...
    outputs, _ = tulip_rs.indicators.linreg.indicator(inputs, options, optional_outputs)

    print(f"Full Linreg Line: {outputs[0]}")
    if optional_count > 0:
        print(f"Full Slope Line: {outputs[1]}")
        print(f"Full Intercept Line: {outputs[2]}")

    ################################################### Calculating the partial LINREG Line
    ...
    outputs2, state2 = tulip_rs.indicators.linreg.indicator(
        inputs2, options, optional_outputs
    )
    print(f"\nPartial Linreg Line: {outputs2[0]}")
    if optional_count > 0:
        print(f"Partial Slope Line: {outputs2[1]}")
        print(f"Partial Intercept Line: {outputs2[2]}")
    ...
    try:
        ...
        print("State info: LINREG State - internal state for Linear Regression")
    ...
    print(f"\nFinal LINREG Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.linreg.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Linreg Line: {full_outputs[0]}")
    if optional_count > 0:
        print(f"Verification Slope Line: {full_outputs[1]}")
        print(f"Verification Intercept Line: {full_outputs[2]}")
    ...
    try:
        # Calculate LINREG for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.linreg.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} LINREG values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.linreg.indicator(
    ...
    try:
        # Calculate LINREG for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.linreg.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} LINREG values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the MSW indicator from tulip_rs_python.

This example demonstrates:
1. Basic MSW calculation
...
def main():
    ...
    info = tulip_rs.indicators.msw.info()
    ...
    min_data = tulip_rs.indicators.msw.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.msw.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full MSW Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.msw.indicator(inputs, options, optional_outputs)

    print(f"FULL MSW Sine: {outputs[0]}")
    print(f"FULL MSW Lead: {outputs[1]}")
    if optional_count > 0:
        for i in range(2, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial MSW Line
    ...
    outputs2, state2 = tulip_rs.indicators.msw.indicator(inputs2, options)
    print(f"\nMSW Sine: {outputs2[0]}")
    print(f"MSW Lead: {outputs2[1]}")
    ...
    try:
        ...
        print("State info: MSW State - internal state for Mesa Sine Wave")
    ...
    print(f"\nNew MSW Sine: {final_outputs[0]}")
    print(f"New MSW Lead: {final_outputs[1]}")
    ...
    full_outputs, _ = tulip_rs.indicators.msw.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification MSW Sine: {full_outputs[0]}")
    print(f"Verification MSW Lead: {full_outputs[1]}")
    if optional_count > 0:
        for i in range(2, len(full_outputs)):
    ...
    try:
        # Calculate MSW for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.msw.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MSW values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.msw.indicator(
    ...
    try:
        # Calculate MSW for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.msw.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} MSW values (first 5): {output[0][:5]}")

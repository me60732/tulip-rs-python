#!/usr/bin/env python3
"""
Python example for the Roofing Filter indicator from tulip_rs_python.

This example demonstrates:
1. Basic roofingfilter calculation with optional outputs
...
def main():
    ...
    info = tulip_rs.indicators.roofingfilter.info()
    ...
    min_data = tulip_rs.indicators.roofingfilter.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Roofing Filter
    optional_outputs = [True] * len(info["optional_outputs"])
    outputs, _ = tulip_rs.indicators.roofingfilter.indicator(
        inputs, options, optional_outputs
    )

    print(f"FULL Roofing Filter: {outputs[0]}")
    print(f"FULL Highpass (optional, may differ in length): {outputs[1]}")

    ################################################### Calculating the Partial Roofing Filter
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.roofingfilter.indicator(
        inputs_partial, options
    )
    print(f"\nRoofing Filter: {outputs_partial[0]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew Roofing Filter: {continued_outputs[0]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.roofingfilter.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Roofing Filter: {full_outputs[0]}")
    print(f"Verification Highpass (optional): {full_outputs[1]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.roofingfilter.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Roofing Filter values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.roofingfilter.indicator(
    ...
    try:
        simd_opt_outputs, simd_opt_states = (
            tulip_rs.indicators.roofingfilter.simd_by_options(
        ...
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(
                f"Option set {i + 1} Roofing Filter values (first 5): {output[0][:5]}"
            )

#!/usr/bin/env python3
"""
Python example for the PSAR indicator from tulip_rs_python.

This example demonstrates:
1. Basic PSAR calculation
...
def main():
    ...
    info = tulip_rs.indicators.psar.info()
    ...
    min_data = tulip_rs.indicators.psar.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.psar.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full PSAR Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.psar.indicator(inputs, options, optional_outputs)

    print(f"Full PSAR Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial PSAR Line
    ...
    outputs2, state2 = tulip_rs.indicators.psar.indicator(inputs2, options)
    print(f"PSAR Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: PSAR State - internal state for Parabolic SAR")
    ...
    full_outputs, _ = tulip_rs.indicators.psar.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification PSAR Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        # Calculate PSAR for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.psar.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} PSAR values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.psar.indicator(
    ...
    try:
        # Calculate PSAR for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.psar.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} PSAR values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the AROONOSC indicator from tulip_rs_python.

This example demonstrates:
1. Basic AROONOSC calculation with optional outputs
...
def main():
    ...
    # Options for AROONOSC: period=5 (matching Rust example)
    options = [5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.aroonosc.info()
    ...
    min_data = tulip_rs.indicators.aroonosc.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.aroonosc.min_data_accuracy(options, 6)
    ...
    # Get optional outputs count and request all (matching Rust example with [true, true])
    outputs, _ = tulip_rs.indicators.aroonosc.indicator(inputs, options, [True, True])

    print("Full dataset calculation:")
    print(f"Full Aroonosc Line: {outputs[0]}")
    print(f"Full Aroon Up Line: {outputs[1]}")
    print(f"Full Aroon Down Line: {outputs[2]}")
    ...
    outputs_partial, state = tulip_rs.indicators.aroonosc.indicator(
        inputs_partial, options
    )

    print(f"\nPartial calculation (first {len(partial_high)} elements):")
    print(f"Aroonosc Line: {outputs_partial[0]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")

    # Get state info
    print(f"State info: AroonOsc State - internal state for Aroon Oscillator")
    ...
    print(f"New AroonOsc Line: {continued_outputs[0]}")
    ...
    try:
        # Calculate AROONOSC for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.aroonosc.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} AROONOSC values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.aroonosc.indicator(
    ...
    try:
        # Calculate AROONOSC for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.aroonosc.simd_by_options(
            expanded_inputs, simd_options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} AROONOSC values (first 5): {output[0][:5]}")

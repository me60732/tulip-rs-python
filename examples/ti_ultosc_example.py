#!/usr/bin/env python3
"""
Python example for the ULTOSC indicator from tulip_rs_python.

This example demonstrates:
1. Basic ULTOSC calculation
...
def main():
    ...
    # Options from Rust example: short_period, medium_period, long_period
    options = [2.0, 3.0, 5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.ultosc.info()
    ...
    min_data = tulip_rs.indicators.ultosc.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.ultosc.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full ULTOSC Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.ultosc.indicator(inputs, options, optional_outputs)

    print(f"Full Ultosc Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial ULTOSC Line
    ...
    outputs2, state2 = tulip_rs.indicators.ultosc.indicator(inputs2, options)
    print(f"\nPartial Ultosc Line: {outputs2[0]}")
    ...
    try:
        ...
        print("State info: ULTOSC State - internal state for Ultimate Oscillator")
    ...
    print(f"\nFinal Ultosc Line: {final_outputs[0]}")
    ...
    full_outputs, _ = tulip_rs.indicators.ultosc.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Ultosc Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.ultosc.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} ULTOSC values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.ultosc.indicator(
    ...
    simd_options = [
        [1.0, 1.5, 2.5],  # Option set 1
        [2.0, 3.0, 5.0],  # Option set 2 (Original)
        [3.0, 4.5, 7.5],  # Option set 3
        [4.0, 6.0, 10.0],  # Option set 4
    ]
    ...
    try:
        # Calculate ULTOSC for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.ultosc.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} ULTOSC values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the VWMA indicator from tulip_rs_python.
...
def main():
    ...
    inputs = [close_vec, volume_vec]

    # Options from Rust example
    options = [5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.vwma.info()
    ...
    min_data = tulip_rs.indicators.vwma.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.vwma.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full VWMA Line
    # Full calculation
    optional_count = (
    ...
    outputs, _ = tulip_rs.indicators.vwma.indicator(inputs, options, optional_outputs)

    print(f"Full VWMA Line: {outputs[0]}")
    if optional_count > 0:
        for i in range(1, len(outputs)):
            print(f"Optional output {i}: {outputs[i]}")

    ################################################### Calculating the partial VWMA Line
    ...
    inputs2 = [close_vec2, volume_vec2]

    # Partial calculation - main outputs only (no optional outputs for state continuation)
    outputs2, state2 = tulip_rs.indicators.vwma.indicator(inputs2, options)
    ...
    try:
        ...
        print(
            "State info: VWMA State - internal state for Volume Weighted Moving Average"
        )
    ...
    full_outputs, _ = tulip_rs.indicators.vwma.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification VWMA Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    simd_inputs = [
        [asset1_close_vec, asset1_volume_vec],  # Asset 1
        [asset2_close_vec, asset2_volume_vec],  # Asset 2
        [asset3_close_vec, asset3_volume_vec],  # Asset 3
        [asset4_close_vec, asset4_volume_vec],  # Asset 4
    ]
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.vwma.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} VWMA values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.vwma.indicator(
    ...
    expanded_inputs = [expanded_close, expanded_volume]
    ...
    try:
        # Calculate VWMA for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.vwma.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} VWMA values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the KAMA indicator from tulip_rs_python.

This example demonstrates:
1. Basic KAMA calculation with optional outputs
...
def main():
    ...
    # Options for KAMA: period=5 (matching Rust example)
    options = [5.0]

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.kama.info()
    ...
    min_data = tulip_rs.indicators.kama.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.kama.min_data_accuracy(options, 6)
    ...
    outputs, _ = tulip_rs.indicators.kama.indicator(inputs, options, optional_outputs)

    print("Full dataset calculation:")
    print(f"Full KAMA Line: {outputs[0]}")
    ...
    outputs_partial, state = tulip_rs.indicators.kama.indicator(inputs_partial, options)

    print(f"\nPartial calculation (first {len(partial_close)} elements):")
    print(f"Partial KAMA Line: {outputs_partial[0]}")
    ...
    # Get state info
    print(
        f"State info: KAMA State - internal state for Kaufman Adaptive Moving Average"
    )
    ...
    print(f"Final Kama Line: {continued_outputs[0]}")
    ...
    try:
        # Calculate KAMA for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.kama.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} KAMA values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.kama.indicator(
    ...
    try:
        # Calculate KAMA for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.kama.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} KAMA values (first 5): {output[0][:5]}")

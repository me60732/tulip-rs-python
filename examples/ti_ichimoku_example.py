#!/usr/bin/env python3
"""
Python example for the Ichimoku Cloud indicator from tulip_rs_python.

This example demonstrates:
1. Basic ichimoku calculation with optional outputs
2. Indicator info display (note: outputs have different lengths due to leading spans)
...
def main():
    ...
    options = [5.0, 10.0]  # short_period, long_period

    ################################################### Show Indicator Info First
    info = tulip_rs.indicators.ichimoku.info()
    ...
    min_data = tulip_rs.indicators.ichimoku.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Ichimoku
    optional_outputs = [True] * len(info["optional_outputs"])
    outputs, _ = tulip_rs.indicators.ichimoku.indicator(
        inputs, options, optional_outputs
    )

    # Note: Ichimoku outputs may have different lengths (leading spans are projected forward)
    all_names = info["outputs"] + info["optional_outputs"]
    print("FULL Ichimoku outputs (name, length, values):")
    for name, output in zip(all_names, outputs):
        print(f"  {name.upper()} (len={len(output)}): {output}")

    ################################################### Calculating the Partial Ichimoku
    inputs_partial = [high[:-5], low[:-5], close[:-5]]
    outputs_partial, state = tulip_rs.indicators.ichimoku.indicator(
        inputs_partial, options
    )
    print(f"\nConversion Line: {outputs_partial[0]}")
    print(f"Base Line: {outputs_partial[1]}")
    print(f"Leading Span A: {outputs_partial[2]}")
    print(f"Leading Span B: {outputs_partial[3]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [high[-5:], low[-5:], close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew Conversion Line: {continued_outputs[0]}")
    print(f"New Base Line: {continued_outputs[1]}")
    print(f"New Leading Span A: {continued_outputs[2]}")
    print(f"New Leading Span B: {continued_outputs[3]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.ichimoku.indicator(
        inputs, options, optional_outputs
    )
    print("Verification Ichimoku outputs (name, length):")
    for name, output in zip(all_names, full_outputs):
        print(f"  {name.upper()} (len={len(output)}): {output}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.ichimoku.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Conversion Line values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.ichimoku.indicator(
    ...
    simd_options = [
        [5.0, 10.0],  # Option set 1: short=5, long=10 (original)
        [7.0, 14.0],  # Option set 2: short=7, long=14
        [9.0, 18.0],  # Option set 3: short=9, long=18
        [9.0, 26.0],  # Option set 4: short=9, long=26 (classic Ichimoku)
    ]

    print(f"Processing {len(simd_options)} option sets simultaneously using SIMD...")
    for i, opt in enumerate(simd_options):
        print(f"Option set {i + 1}: short_period={opt[0]}, long_period={opt[1]}")
    print()

    try:
        simd_opt_outputs, simd_opt_states = (
            tulip_rs.indicators.ichimoku.simd_by_options(
        ...
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} Conversion Line (first 5): {output[0][:5]}")

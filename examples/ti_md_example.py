#!/usr/bin/env python3
"""
Python example for the MD indicator from tulip_rs_python.
...
def main():
    ...
    info = tulip_rs.indicators.md.info()
    ...
    min_data = tulip_rs.indicators.md.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.md.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full MD Line with Optional Outputs
    ...
    outputs, _ = tulip_rs.indicators.md.indicator(inputs, options, optional_outputs)

    print(f"Full MD Line: {outputs[0]}")
    if optional_count > 0:
        print(f"Full SMA Line: {outputs[1]}")

    ################################################### Calculating the partial MD Line
    ...
    outputs2, state2 = tulip_rs.indicators.md.indicator(
    ...
    try:
        ...
        print("State info: MD State - internal state for Mean Deviation")
    ...
    full_outputs, _ = tulip_rs.indicators.md.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification MD Line: {full_outputs[0]}")
    if optional_count > 0:
    ...
    try:
        # Calculate MD for all assets using SIMD
        simd_outputs, simd_states = tulip_rs.indicators.md.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} MD values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.md.indicator(
    ...
    try:
        # Calculate MD for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.md.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} MD values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the ADX indicator from tulip_rs_python.

This example demonstrates:
1. Basic ADX calculation
...
def main():
    ...
    info = tulip_rs.indicators.adx.info()
    ...
    min_data = tulip_rs.indicators.adx.min_data(options)
    print(f"Minimum data required: {min_data}")

    # Show minimum data for accuracy
    min_data_accuracy = tulip_rs.indicators.adx.min_data_accuracy(options, 6)
    print(f"Minimum data for accuracy (6 decimals): {min_data_accuracy}")
    print()

    ################################################### Calculating the Full ADX Line with Optional Outputs
    ...
    outputs, _ = tulip_rs.indicators.adx.indicator(inputs, options, optional_outputs)

    print(f"Full adx line: {outputs[0]}")
    if optional_count > 0:
        print(f"Full dx line: {outputs[1]}")
        print(f"Full atr line: {outputs[2]}")
        print(f"Full tr line: {outputs[3]}")

    ################################################### Calculating the partial ADX Line
    ...
    outputs2, state2 = tulip_rs.indicators.adx.indicator(inputs2, options)
    ...
    try:
        ...
        print("State info: ADX State - internal state for Average Directional Index")
    ...
    full_outputs, _ = tulip_rs.indicators.adx.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification adx line: {full_outputs[0]}")
    if optional_count > 0:
        print(f"Verification dx line: {full_outputs[1]}")
        print(f"Verification atr line: {full_outputs[2]}")
        print(f"Verification tr line: {full_outputs[3]}")
    ...
    asset1_close = np.array(close, dtype=np.float64)
    ...
    asset2_close = np.array([v * 1.2 for v in close], dtype=np.float64)
    ...
    simd_inputs = [
        [asset1_high, asset1_low, asset1_close],  # Asset 1
        [asset2_high, asset2_low, asset2_close],  # Asset 2
        [asset3_high, asset3_low, asset3_close],  # Asset 3
        [asset4_high, asset4_low, asset4_close],  # Asset 4
    ]

    print(f"Processing {len(simd_inputs)} assets simultaneously using SIMD...")
    print("Asset 1: Original data")
    print("Asset 2: Scaled up (+20% prices)")
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.adx.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} ADX values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.adx.indicator(
    ...
    try:
        # Calculate ADX for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.adx.simd_by_options(
            expanded_inputs, simd_options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} ADX values (first 5): {output[0][:5]}")

#!/usr/bin/env python3
"""
Python example for the Highpass Filter indicator from tulip_rs_python.

This example demonstrates:
1. Basic highpass calculation (no optional outputs)
...
def main():
    ...
    info = tulip_rs.indicators.highpass.info()
    ...
    min_data = tulip_rs.indicators.highpass.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Highpass
    # No optional outputs for this indicator
    optional_outputs = None
    outputs, _ = tulip_rs.indicators.highpass.indicator(
        inputs, options, optional_outputs
    )

    print(f"FULL Highpass: {outputs[0]}")

    ################################################### Calculating the Partial Highpass
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.highpass.indicator(
        inputs_partial, options
    )
    print(f"\nHighpass: {outputs_partial[0]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew Highpass: {continued_outputs[0]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.highpass.indicator(inputs, options)
    print(f"Verification Highpass: {full_outputs[0]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.highpass.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Highpass values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.highpass.indicator(
    ...
    simd_options = [
        [10.0],  # Option set 1: period=10
        [20.0],  # Option set 2: period=20 (original)
        [30.0],  # Option set 3: period=30
        [40.0],  # Option set 4: period=40
    ]
    ...
    try:
        simd_opt_outputs, simd_opt_states = (
            tulip_rs.indicators.highpass.simd_by_options(expanded_inputs, simd_options)
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} Highpass values (first 5): {output[0][:5]}")

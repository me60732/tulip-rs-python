...
def main():
    ...
    simd_inputs = [
        [asset1_close],  # Asset 1
        [asset2_close],  # Asset 2
        [asset3_close],  # Asset 3
        [asset4_close],  # Asset 4
    ]
    ...
    try:
        ...
        simd_outputs, simd_states = tulip_rs.indicators.cmo.simd_by_assets(
            simd_inputs, options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} CMO values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.cmo.indicator(
    ...
    expanded_new_close = np.tile(new_close, 20).astype(np.float64)
    expanded_inputs = [expanded_new_close]
    ...
    try:
        # Calculate CMO for all option sets using SIMD
        simd_opt_outputs, simd_opt_states = tulip_rs.indicators.cmo.simd_by_options(
            expanded_inputs, simd_options
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} CMO values (first 5): {output[0][:5]}")

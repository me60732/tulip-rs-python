#!/usr/bin/env python3
"""
Python example for the Trendmode indicator from tulip_rs_python.

This example demonstrates:
1. Basic trendmode calculation with optional outputs
...
def main():
    ...
    info = tulip_rs.indicators.trendmode.info()
    ...
    min_data = tulip_rs.indicators.trendmode.min_data(options)
    print(f"Minimum data required: {min_data}")
    print()

    ################################################### Calculating the Full Trendmode
    optional_outputs = [True] * len(info["optional_outputs"])
    outputs, _ = tulip_rs.indicators.trendmode.indicator(
        inputs, options, optional_outputs
    )

    print(f"FULL Trendmode: {outputs[0]}")
    print(f"FULL Cycle (optional): {outputs[1]}")
    print(f"FULL Peak (optional): {outputs[2]}")

    ################################################### Calculating the Partial Trendmode
    inputs_partial = [close[:-5]]
    outputs_partial, state = tulip_rs.indicators.trendmode.indicator(
        inputs_partial, options
    )
    print(f"\nTrendmode: {outputs_partial[0]}")

    ################################################### State Continuation Demo
    print("\nDemonstrating state continuation...")
    new_data = [close[-5:]]
    continued_outputs = state.batch_indicator(new_data)
    print(f"\nNew Trendmode: {continued_outputs[0]}")

    print("\nVerification - calculating full sequence:")
    full_outputs, _ = tulip_rs.indicators.trendmode.indicator(
        inputs, options, optional_outputs
    )
    print(f"Verification Trendmode: {full_outputs[0]}")
    print(f"Verification Cycle (optional): {full_outputs[1]}")
    print(f"Verification Peak (optional): {full_outputs[2]}")
    ...
    try:
        simd_outputs, simd_states = tulip_rs.indicators.trendmode.simd_by_assets(
            simd_inputs, options, optional_outputs
        )

        print("SIMD Results:")
        for i, (output, state) in enumerate(zip(simd_outputs, simd_states)):
            print(f"Asset {i + 1} Trendmode values: {output[0]}")

        print("\nVerification - calculating each asset individually:")
        for i, asset_inputs in enumerate(simd_inputs):
            individual_output, _ = tulip_rs.indicators.trendmode.indicator(
    ...
    try:
        simd_opt_outputs, simd_opt_states = (
            tulip_rs.indicators.trendmode.simd_by_options(
        ...
        for i, (output, state) in enumerate(zip(simd_opt_outputs, simd_opt_states)):
            print(f"Option set {i + 1} Trendmode values (first 5): {output[0][:5]}")

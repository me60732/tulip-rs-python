# Benchmark: zlema (Zero-Lag Exponential Moving Average)
...
def _tulip(data: OhlcvArrays, options: List[float]) -> Any:
    return tulip_rs.indicators.zlema.indicator([data.close], options)
...
def _simd_assets(stocks: List[OhlcvArrays], options: List[float]) -> Any:
    ...
    return tulip_rs.indicators.zlema.simd_by_assets(inputs, options, None)
...
def _simd_options(data: OhlcvArrays, options_list: List[List[float]]) -> Any:
    ...
    return tulip_rs.indicators.zlema.simd_by_options([data.close], options_list, None)
...
def _pta(data: OhlcvArrays, options: List[float]) -> Any:
    return pta.zlma(pd.Series(data.close), length=int(options[0]))

# Benchmark: linreg (Linear Regression)
...
def _tulip(data: OhlcvArrays, options: List[float]) -> Any:
    return tulip_rs.indicators.linreg.indicator([data.close], options)


def _pta(data: OhlcvArrays, options: List[float]) -> Any:
    return pta.linreg(pd.Series(data.close), length=int(options[0]))
...
def _simd_assets(stocks: List[OhlcvArrays], options: List[float]) -> Any:
    ...
    return tulip_rs.indicators.linreg.simd_by_assets(inputs, options, None)
...
def _simd_options(data: OhlcvArrays, options_list: List[List[float]]) -> Any:
    ...
    return tulip_rs.indicators.linreg.simd_by_options([data.close], options_list, None)

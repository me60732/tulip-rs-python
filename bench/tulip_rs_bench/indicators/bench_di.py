# Benchmark: di (Directional Indicator)
...
def _tulip(data: OhlcvArrays, options: List[float]) -> Any:
    return tulip_rs.indicators.di.indicator([data.high, data.low, data.close], options)


def _pta(data: OhlcvArrays, options: List[float]) -> Any:
    return pta.dm(pd.Series(data.high), pd.Series(data.low), length=int(options[0]))
...
def _simd_assets(stocks: List[OhlcvArrays], options: List[float]) -> Any:
    ...
    return tulip_rs.indicators.di.simd_by_assets(inputs, options, None)
...
def _simd_options(data: OhlcvArrays, options_list: List[List[float]]) -> Any:
    ...
    return tulip_rs.indicators.di.simd_by_options([data.high, data.low, data.close], options_list, None)

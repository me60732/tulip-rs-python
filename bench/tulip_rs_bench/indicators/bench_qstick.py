...
def _tulip(data: OhlcvArrays, options: List[float]) -> Any:
    return tulip_rs.indicators.qstick.indicator([data.open, data.close], options)
...
def _simd_assets(stocks: List[OhlcvArrays], options: List[float]) -> Any:
    ...
    inputs = [[stock.open, stock.close] for stock in stocks]
    return tulip_rs.indicators.qstick.simd_by_assets(inputs, options, None)
...
def _simd_options(data: OhlcvArrays, options_list: List[List[float]]) -> Any:
    ...
    return tulip_rs.indicators.qstick.simd_by_options([data.open, data.close], options_list, None)

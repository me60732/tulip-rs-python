...
use tulip_rs::indicators::ef::{
    Ef, Indicator, IndicatorByOptions, IndicatorState, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, EfState)> {
    if inputs.len() != INPUTS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            INPUTS,
    ...
    match Ef::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Ef::INFO)
}
...
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(Ef::min_data(&options_array))
}

/// Calculate EF (Efficiency Ratio) for multiple assets using SIMD operations
///
...
/// the net change over a period to the total absolute price changes during that period.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [real] array
/// - options: Vector containing [period] for the lookback period
/// - optional_outputs: Optional list of booleans for additional outputs (none available for EF)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of EF results for each asset (each asset returns one efficiency ratio line)
///   - states: Vector of EfState objects for continuing calculations
///
/// Input Structure:
...
/// real1 = np.array([10.0, 10.5, 10.3, 10.8, 11.0, 10.9, 11.2, 11.1, 11.3, 11.0,
///                   10.8, 11.1, 11.4, 11.2, 11.5, 11.3, 11.6, 11.4, 11.7, 11.5], dtype=np.float64)
///
...
///
/// # EF options: [period]
/// options = [5.0]  # Lookback period = 5
///
/// # Calculate EF for all assets using SIMD
/// outputs, states = ti.indicators.ef_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<EfState>)> {
    if inputs.is_empty() {
    ...
    let result = match num_assets {
        2 => {
            ...
            Ef::indicator_by_assets::<2>(input_array, &options_array, optional_outputs.as_deref())
        }
        4 => {
            let input_array: &[&[&[f64]; INPUTS]; 4] = input_refs.as_slice().try_into().unwrap();
            Ef::indicator_by_assets::<4>(input_array, &options_array, optional_outputs.as_deref())
        }
        8 => {
            let input_array: &[&[&[f64]; INPUTS]; 8] = input_refs.as_slice().try_into().unwrap();
            Ef::indicator_by_assets::<8>(input_array, &options_array, optional_outputs.as_deref())
        }
        16 => {
            let input_array: &[&[&[f64]; INPUTS]; 16] = input_refs.as_slice().try_into().unwrap();
            Ef::indicator_by_assets::<16>(input_array, &options_array, optional_outputs.as_deref())
        }
    ...
    match result {
        Ok((results, states)) => {
            let ef_states: Vec<EfState> = states
                .into_iter()
                .map(|state| EfState { inner: state })
                .collect();
            Ok((crate::utils::simd_vecs_to_pyarrays(py, results), ef_states))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
/// Calculate EF (Efficiency Ratio) for a single asset with multiple options using SIMD
///
/// Parameters:
/// - inputs: List of numpy array [real]
...
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of EF results for each option set
///   - states: Vector of EfState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            Ef::indicator_by_options::<2>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        4 => {
            let opt_array: &[&[f64; OPTIONS]; 4] = option_refs.as_slice().try_into().unwrap();
            Ef::indicator_by_options::<4>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        8 => {
            let opt_array: &[&[f64; OPTIONS]; 8] = option_refs.as_slice().try_into().unwrap();
            Ef::indicator_by_options::<8>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        16 => {
            let opt_array: &[&[f64; OPTIONS]; 16] = option_refs.as_slice().try_into().unwrap();
            Ef::indicator_by_options::<16>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        _ => unreachable!("Already validated SIMD lane count"),
    };
...
/// Register the EF indicator module with Python
///
/// This function creates a Python submodule for the EF indicator and registers
...
pub fn register_ef_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "ef")?;

...
use tulip_rs::indicators::avgprice::{
    AvgPrice, Indicator, IndicatorState, TIndicatorState, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, AvgpriceState)> {
    if inputs.len() != INPUTS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            INPUTS,
    ...
    match AvgPrice::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
            crate::utils::vecs_to_pyarrays(py, result),
            AvgpriceState { inner: state },
        )),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, AvgPrice::INFO)
}
...
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    ...
    let options_array: [f64; OPTIONS] = [];
    Ok(AvgPrice::min_data(&options_array))
}
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<AvgpriceState>)> {
    if inputs.is_empty() {
    ...
    let result = match num_assets {
        2 => {
            ...
            AvgPrice::indicator_by_assets::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            ...
            AvgPrice::indicator_by_assets::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            AvgPrice::indicator_by_assets::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            AvgPrice::indicator_by_assets::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )

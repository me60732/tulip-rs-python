...
use tulip_rs::indicators::vwap::{
    Indicator, IndicatorState, TIndicatorState, Vwap, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    match Vwap::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Vwap::INFO)
}
...
/// Calculate VWAP (Volume Weighted Average Price) for multiple assets using SIMD operations
///
/// This function processes multiple assets simultaneously for improved performance
/// using SIMD (Single Instruction, Multiple Data) operations.
///
/// The Volume Weighted Average Price is a cumulative indicator that divides the
/// cumulative typical price times volume by the cumulative volume. It's commonly
/// used to determine the average price an asset has traded at over a period.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [high, low, close, volume] arrays
/// - options: Empty vector (VWAP has no options)
/// - optional_outputs: Optional list of booleans for additional outputs (typprice available)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of VWAP results for each asset (each asset returns vwap line and optionally typprice)
///   - states: Vector of VwapState objects for continuing calculations
///
/// Input Structure:
...
/// ]
///
/// # VWAP has no options (empty list)
/// options = []
///
/// # Calculate VWAP for all assets using SIMD
/// outputs, states = ti.indicators.vwap_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    let result = match num_assets {
        2 => {
            ...
            Vwap::indicator_by_assets::<2>(input_array, &options_array, optional_outputs.as_deref())
        }
        4 => {
            let input_array: &[&[&[f64]; INPUTS]; 4] = input_refs.as_slice().try_into().unwrap();
            Vwap::indicator_by_assets::<4>(input_array, &options_array, optional_outputs.as_deref())
        }
        8 => {
            let input_array: &[&[&[f64]; INPUTS]; 8] = input_refs.as_slice().try_into().unwrap();
            Vwap::indicator_by_assets::<8>(input_array, &options_array, optional_outputs.as_deref())
        }
        16 => {
            let input_array: &[&[&[f64]; INPUTS]; 16] = input_refs.as_slice().try_into().unwrap();
            Vwap::indicator_by_assets::<16>(
                input_array,
                &options_array,
...
/// Register the VWAP indicator module with Python
///
/// This function creates a Python submodule for the VWAP indicator and registers

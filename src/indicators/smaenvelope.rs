...
use tulip_rs::indicators::smaenvelope::{
    Indicator, IndicatorByOptions, IndicatorState, SmaEnvelope, TIndicatorState, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    match SmaEnvelope::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, SmaEnvelope::INFO)
}

#[pyfunction]
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    let options_array: [f64; OPTIONS] = [options[0], options[1]];
    Ok(SmaEnvelope::min_data(&options_array))
}

/// Calculate SMA Envelope for multiple assets using SIMD operations
///
/// This function processes multiple assets simultaneously for improved performance
/// using SIMD (Single Instruction, Multiple Data) operations.
///
/// Parameters:
/// - inputs: Vector of asset inputs for SMA Envelope calculation
/// - options: Vector of options for SMA Envelope calculation
/// - optional_outputs: Optional list of booleans for additional outputs
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of SMA Envelope results for each asset
///   - states: Vector of SmaEnvelopeState objects for continuing calculations
///
...
pub fn simd_by_assets(
    ...
    let result = match num_assets {
        2 => {
            ...
            SmaEnvelope::indicator_by_assets::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            ...
            SmaEnvelope::indicator_by_assets::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            SmaEnvelope::indicator_by_assets::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            SmaEnvelope::indicator_by_assets::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
    ...
    match result {
        Ok((results, states)) => {
            let smaenvelope_states: Vec<SmaEnvelopeState> = states
                .into_iter()
...
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            SmaEnvelope::indicator_by_options::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            ...
            SmaEnvelope::indicator_by_options::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            SmaEnvelope::indicator_by_options::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            SmaEnvelope::indicator_by_options::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )

...
/// - options: Vector containing [period] for the ChaikinMF calculation
/// - optional_outputs: Not used (ChaikinMF has no optional outputs)
///
/// Returns:
...
pub fn simd_by_assets(
    ...
    match result {
        Ok((results, states)) => {
            let chaikinmf_states: Vec<ChaikinmfState> = states
                .into_iter()
...
pub fn register_chaikinmf_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    ...
    submodule.add_class::<ChaikinmfState>()?;
...
/// Get minimum data length required for ChaikinMF calculation with accuracy
#[pyfunction]
pub fn min_data_accuracy(options: Vec<f64>, decimals: usize) -> PyResult<usize> {
    ...
    Ok(rust_chaikinmf::min_data_accuracy(&options, decimals))
}

/// Get output length for ChaikinMF calculation
#[pyfunction]
pub fn output_length(data_len: usize, options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(rust_chaikinmf::output_length(data_len, &options))
}

...
use tulip_rs::indicators::trendmode::{
    Indicator, IndicatorByOptions, IndicatorState, TrendMode, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    match TrendMode::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, TrendMode::INFO)
}

#[pyfunction]
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    let options_array: [f64; OPTIONS] = [options[0]];
    Ok(TrendMode::min_data(&options_array))
}

/// Calculate Trendmode for multiple assets using SIMD operations
///
...
/// - options: Vector containing [alpha] for the filter coefficient
/// - optional_outputs: Optional list of booleans for additional outputs (cycle, peak)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Trendmode results for each asset
///   - states: Vector of TrendmodeState objects for continuing calculations
///
...
pub fn simd_by_assets(
    ...
    let result = match num_assets {
        2 => {
            ...
            TrendMode::indicator_by_assets::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            ...
            TrendMode::indicator_by_assets::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            TrendMode::indicator_by_assets::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            TrendMode::indicator_by_assets::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
...
/// Calculate Trendmode for a single asset with multiple options using SIMD
///
/// Parameters:
/// - inputs: List of numpy arrays [real]
/// - options: List of option arrays, where each array contains [alpha]
/// - optional_outputs: Optional list of booleans for additional outputs
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Trendmode results for each option set
///   - states: Vector of TrendmodeState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            TrendMode::indicator_by_options::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            ...
            TrendMode::indicator_by_options::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            TrendMode::indicator_by_options::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            TrendMode::indicator_by_options::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )

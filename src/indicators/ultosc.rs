...
use tulip_rs::indicators::ultosc::{
    Indicator, IndicatorByOptions, IndicatorState, Ultosc, INPUTS, OPTIONS, TIndicatorState
};
...
pub fn indicator(
    ...
    match Ultosc::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Ultosc::INFO)
}
...
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(Ultosc::min_data(&options_array))
}
...
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            Ultosc::indicator_by_options::<2>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        4 => {
            let opt_array: &[&[f64; OPTIONS]; 4] = option_refs.as_slice().try_into().unwrap();
            Ultosc::indicator_by_options::<4>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        8 => {
            let opt_array: &[&[f64; OPTIONS]; 8] = option_refs.as_slice().try_into().unwrap();
            Ultosc::indicator_by_options::<8>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        16 => {
            let opt_array: &[&[f64; OPTIONS]; 16] = option_refs.as_slice().try_into().unwrap();
            Ultosc::indicator_by_options::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )

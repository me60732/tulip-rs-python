...
pub fn indicator(
    ...
    if options.len() != rust_mom::OPTIONS_WIDTH {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} options, got {}",
            rust_mom::OPTIONS_WIDTH,
            options.len()
        )));
    }
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, rust_mom::INFO)
}

#[pyfunction]
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_mom::min_data(&options))
}

#[pyfunction]
pub fn min_data_accuracy(options: Vec<f64>, decimals: usize) -> PyResult<usize> {
    Ok(rust_mom::min_data_accuracy(&options, decimals))
}

#[pyfunction]
pub fn output_length(data_len: usize, options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_mom::output_length(data_len, &options))
}
...
pub fn simd_by_assets(
    ...
    for (asset_idx, asset_inputs) in inputs.iter().enumerate() {
        if asset_inputs.len() != rust_mom::INPUTS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let mut asset_input_arrays: Vec<[&[f64]; rust_mom::INPUTS_WIDTH]> =
        Vec::with_capacity(num_assets);

    for asset_inputs in &inputs {
        let input_array: Result<[&[f64]; rust_mom::INPUTS_WIDTH], _> = asset_inputs
            .iter()
    ...
    let input_refs: Vec<&[&[f64]; rust_mom::INPUTS_WIDTH]> = asset_input_arrays.iter().collect();

    let options_array: Result<[f64; rust_mom::OPTIONS_WIDTH], _> = options.try_into();
    ...
    let result = match num_assets {
        2 => {
            let input_array: &[&[&[f64]; rust_mom::INPUTS_WIDTH]; 2] =
                input_refs.as_slice().try_into().unwrap();
            rust_mom::by_assets::indicator::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            let input_array: &[&[&[f64]; rust_mom::INPUTS_WIDTH]; 4] =
                input_refs.as_slice().try_into().unwrap();
            rust_mom::by_assets::indicator::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let input_array: &[&[&[f64]; rust_mom::INPUTS_WIDTH]; 8] =
                input_refs.as_slice().try_into().unwrap();
            rust_mom::by_assets::indicator::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let input_array: &[&[&[f64]; rust_mom::INPUTS_WIDTH]; 16] =
                input_refs.as_slice().try_into().unwrap();
            rust_mom::by_assets::indicator::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
    ...
    match result {
        Ok((results, states)) => {
            let mom_states: Vec<MomState> = states
                .into_iter()
...
pub fn simd_by_options(
    ...
    for (opt_idx, opt) in options.iter().enumerate() {
        if opt.len() != rust_mom::OPTIONS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let option_refs: Vec<&[f64; rust_mom::OPTIONS_WIDTH]> = option_arrays.iter().collect();

    // Call the SIMD by options function with proper const generic
    let result = match num_options {
        2 => {
            let opt_array: &[&[f64; rust_mom::OPTIONS_WIDTH]; 2] =
                option_refs.as_slice().try_into().unwrap();
            rust_mom::by_options::indicator::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            let opt_array: &[&[f64; rust_mom::OPTIONS_WIDTH]; 4] =
                option_refs.as_slice().try_into().unwrap();
            rust_mom::by_options::indicator::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let opt_array: &[&[f64; rust_mom::OPTIONS_WIDTH]; 8] =
                option_refs.as_slice().try_into().unwrap();
            rust_mom::by_options::indicator::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let opt_array: &[&[f64; rust_mom::OPTIONS_WIDTH]; 16] =
                option_refs.as_slice().try_into().unwrap();
            rust_mom::by_options::indicator::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
...
pub fn register_mom_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "mom")?;

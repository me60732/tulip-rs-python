...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<AdaptivemswState>)> {
    if inputs.is_empty() {
    ...
    match result {
        Ok((results, states)) => {
            let adaptivemsw_states: Vec<AdaptivemswState> = states
                .into_iter()
                .map(|state| AdaptivemswState { inner: state })
                .collect();
            Ok((
...
pub fn register_adaptivemsw_module(
    parent_module: &pyo3::Bound<'_, PyModule>,
) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "adaptivemsw")?;

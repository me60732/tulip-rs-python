...
///
/// The Aroon Oscillator is the difference between the Aroon Up and Aroon Down indicators.
/// It oscillates between -100 and +100, with positive values indicating upward momentum
/// and negative values indicating downward momentum.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [high, low] arrays
/// - options: Vector containing [period] for the Aroon Oscillator calculation
/// - optional_outputs: Optional list of booleans for additional outputs (none available for AROONOSC)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of AROONOSC results for each asset (each asset returns one oscillator line)
///   - states: Vector of AroonoscState objects for continuing calculations
///
/// Input Structure:
...
/// ]
///
/// # AROONOSC options: [period]
/// options = [14.0]  # 14-period Aroon Oscillator
///
/// # Calculate AROONOSC for all assets using SIMD
/// outputs, states = ti.indicators.aroonosc_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Vec<f64>>>, Vec<AroonoscState>)> {
    if inputs.is_empty() {
    ...
    match result {
        Ok((results, states)) => {
            let aroonosc_states: Vec<AroonoscState> = states
                .into_iter()
                .map(|state| AroonoscState { inner: state })
                .collect();
            Ok((results, aroonosc_states))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
pub fn simd_by_options(
    ...
    let mut option_arrays: Vec<[f64; rust_aroonosc::OPTIONS_WIDTH]> = Vec::with_capacity(num_options);
...
pub fn register_aroonosc_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "aroonosc")?;
    ...
    submodule.add_class::<AroonoscState>()?;

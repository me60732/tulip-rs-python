...
impl DxState {
    ...
    fn batch_indicator(
        ...
        if inputs.len() != rust_dx::INPUTS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                "Expected {} inputs, got {}",
                rust_dx::INPUTS_WIDTH,
                inputs.len()
            )));
        }

        let input_arrays: [&[f64]; rust_dx::INPUTS_WIDTH] = [
            inputs[0].as_slice()?,
            inputs[1].as_slice()?,
            inputs[2].as_slice()?,
        ];
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, DxState)> {
    if inputs.len() != rust_dx::INPUTS_WIDTH {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            rust_dx::INPUTS_WIDTH,
    ...
    if options.len() != rust_dx::OPTIONS_WIDTH {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} options, got {}",
            rust_dx::OPTIONS_WIDTH,
            options.len()
        )));
    }
    ...
    let options_array: [f64; rust_dx::OPTIONS_WIDTH] = [options[0]];

    match rust_dx::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, rust_dx::INFO)
}

#[pyfunction]
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_dx::min_data(&options))
}

#[pyfunction]
pub fn min_data_accuracy(options: Vec<f64>, decimals: usize) -> PyResult<usize> {
    Ok(rust_dx::min_data_accuracy(&options, decimals))
}

#[pyfunction]
pub fn output_length(data_len: usize, options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_dx::output_length(data_len, &options))
}

/// Calculate DX (Directional Index) for multiple assets using SIMD operations
///
/// This function processes multiple assets simultaneously for improved performance
/// using SIMD (Single Instruction, Multiple Data) operations.
///
/// The Directional Index (DX) measures the difference between the positive and
/// negative directional indicators, providing a single value that indicates the
/// strength of the directional movement regardless of direction.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [high, low, close] arrays
/// - options: Vector containing [period] for the DX calculation
/// - optional_outputs: Optional list of booleans for additional outputs
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of DX results for each asset (each asset returns one DX line)
///   - states: Vector of DxState objects for continuing calculations
///
/// Input Structure:
...
///
/// # DX options: [period]
/// options = [14.0]  # 14-period DX
///
/// # Calculate DX for all assets using SIMD
/// outputs, states = ti.indicators.dx_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<DxState>)> {
    if inputs.is_empty() {
    ...
    for (asset_idx, asset_inputs) in inputs.iter().enumerate() {
        if asset_inputs.len() != rust_dx::INPUTS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let mut asset_input_arrays: Vec<[&[f64]; rust_dx::INPUTS_WIDTH]> =
        Vec::with_capacity(num_assets);

    for asset_inputs in &inputs {
        let input_array: [&[f64]; rust_dx::INPUTS_WIDTH] = [
    ...
    let input_refs: Vec<&[&[f64]; rust_dx::INPUTS_WIDTH]> = asset_input_arrays.iter().collect();
    ...
    let result = match num_assets {
        2 => {
            let input_array: &[&[&[f64]; rust_dx::INPUTS_WIDTH]; 2] =
                input_refs.as_slice().try_into().unwrap();
            rust_dx::by_assets::indicator::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            let input_array: &[&[&[f64]; rust_dx::INPUTS_WIDTH]; 4] =
                input_refs.as_slice().try_into().unwrap();
            rust_dx::by_assets::indicator::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let input_array: &[&[&[f64]; rust_dx::INPUTS_WIDTH]; 8] =
                input_refs.as_slice().try_into().unwrap();
            rust_dx::by_assets::indicator::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let input_array: &[&[&[f64]; rust_dx::INPUTS_WIDTH]; 16] =
                input_refs.as_slice().try_into().unwrap();
            rust_dx::by_assets::indicator::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
    ...
    match result {
        Ok((results, states)) => {
            let dx_states: Vec<DxState> = states
                .into_iter()
                .map(|state| DxState { inner: state })
                .collect();
            Ok((crate::utils::simd_vecs_to_pyarrays(py, results), dx_states))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
// Auto-register functions for DX
...
pub fn simd_by_options(
    ...
    for (opt_idx, opt) in options.iter().enumerate() {
        if opt.len() != rust_dx::OPTIONS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let mut option_arrays: Vec<[f64; rust_dx::OPTIONS_WIDTH]> = Vec::with_capacity(num_options);

    for opt in &options {
        option_arrays.push([opt[0]]);
    }

    let option_refs: Vec<&[f64; rust_dx::OPTIONS_WIDTH]> = option_arrays.iter().collect();

    // Call the SIMD by options function with proper const generic
    let result = match num_options {
        2 => {
            let opt_array: &[&[f64; rust_dx::OPTIONS_WIDTH]; 2] =
                option_refs.as_slice().try_into().unwrap();
            rust_dx::by_options::indicator::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            let opt_array: &[&[f64; rust_dx::OPTIONS_WIDTH]; 4] =
                option_refs.as_slice().try_into().unwrap();
            rust_dx::by_options::indicator::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let opt_array: &[&[f64; rust_dx::OPTIONS_WIDTH]; 8] =
                option_refs.as_slice().try_into().unwrap();
            rust_dx::by_options::indicator::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let opt_array: &[&[f64; rust_dx::OPTIONS_WIDTH]; 16] =
                option_refs.as_slice().try_into().unwrap();
            rust_dx::by_options::indicator::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
...
pub fn register_dx_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "dx")?;

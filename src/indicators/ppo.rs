...
use tulip_rs::indicators::ppo::{
    Indicator, IndicatorByOptions, IndicatorState, Ppo, TIndicatorState, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, PpoState)> {
    if inputs.len() != INPUTS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            INPUTS,
    ...
    match Ppo::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
            crate::utils::vecs_to_pyarrays(py, result),
            PpoState { inner: state },
        )),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Ppo::INFO)
}

#[pyfunction]
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    let options_array: [f64; OPTIONS] = [options[0], options[1]];
    Ok(Ppo::min_data(&options_array))
}

/// Calculate PPO for multiple assets using SIMD operations
///
/// This function processes multiple assets simultaneously for improved performance
/// using SIMD (Single Instruction, Multiple Data) operations.
///
/// Parameters:
/// - inputs: Vector of asset inputs for PPO calculation
/// - options: Vector of options for PPO calculation
/// - optional_outputs: Optional list of booleans for additional outputs [short_ema, long_ema]
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of PPO results for each asset (each asset returns ppo + optional outputs)
///   - states: Vector of PpoState objects for continuing calculations
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<PpoState>)> {
    if inputs.is_empty() {
    ...
    let result = match num_assets {
        2 => {
            ...
            Ppo::indicator_by_assets::<2>(input_array, &options_array, optional_outputs.as_deref())
        }
        4 => {
            let input_array: &[&[&[f64]; INPUTS]; 4] = input_refs.as_slice().try_into().unwrap();
            Ppo::indicator_by_assets::<4>(input_array, &options_array, optional_outputs.as_deref())
        }
        8 => {
            let input_array: &[&[&[f64]; INPUTS]; 8] = input_refs.as_slice().try_into().unwrap();
            Ppo::indicator_by_assets::<8>(input_array, &options_array, optional_outputs.as_deref())
        }
        16 => {
            let input_array: &[&[&[f64]; INPUTS]; 16] = input_refs.as_slice().try_into().unwrap();
            Ppo::indicator_by_assets::<16>(input_array, &options_array, optional_outputs.as_deref())
        }
    ...
    match result {
        Ok((results, states)) => {
            let ppo_states: Vec<PpoState> = states
                ...
                .collect();
            Ok((crate::utils::simd_vecs_to_pyarrays(py, results), ppo_states))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
/// Calculate PPO for a single asset with multiple options using SIMD
///
/// Parameters:
...
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of PPO results for each option set
///   - states: Vector of PpoState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            Ppo::indicator_by_options::<2>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        4 => {
            let opt_array: &[&[f64; OPTIONS]; 4] = option_refs.as_slice().try_into().unwrap();
            Ppo::indicator_by_options::<4>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        8 => {
            let opt_array: &[&[f64; OPTIONS]; 8] = option_refs.as_slice().try_into().unwrap();
            Ppo::indicator_by_options::<8>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        16 => {
            let opt_array: &[&[f64; OPTIONS]; 16] = option_refs.as_slice().try_into().unwrap();
            Ppo::indicator_by_options::<16>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        _ => unreachable!("Already validated SIMD lane count"),
    };

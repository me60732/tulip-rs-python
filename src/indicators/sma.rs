...
use tulip_rs::indicators::sma::{
    Indicator, IndicatorByOptions, IndicatorState, Sma, TIndicatorState, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, SmaState)> {
    if inputs.len() != INPUTS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            INPUTS,
    ...
    match Sma::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
            crate::utils::vecs_to_pyarrays(py, result),
            SmaState { inner: state },
        )),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Sma::INFO)
}
...
///
/// The Simple Moving Average calculates the arithmetic mean of a sequence of values
/// over a specified period. It's a trend-following indicator that smooths out price
/// data to form a trend.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains a single [real] array
/// - options: Vector containing the period for the moving average calculation
/// - optional_outputs: Optional list of booleans for additional outputs (none available for SMA)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of SMA results for each asset (each asset returns one average line)
...
/// real1 = np.array([10.5, 10.8, 11.0, 10.9, 11.2, 11.1, 11.3, 11.0, 10.8, 11.1, 11.4, 11.2, 11.5, 11.3, 11.6], dtype=np.float64)
/// real2 = np.array([20.5, 20.8, 21.0, 20.9, 21.2, 21.1, 21.3, 21.0, 20.8, 21.1, 21.4, 21.2, 21.5, 21.3, 21.6], dtype=np.float64)
/// real3 = np.array([30.5, 30.8, 31.0, 30.9, 31.2, 31.1, 31.3, 31.0, 30.8, 31.1, 31.4, 31.2, 31.5, 31.3, 31.6], dtype=np.float64)
/// real4 = np.array([40.5, 40.8, 41.0, 40.9, 41.2, 41.1, 41.3, 41.0, 40.8, 41.1, 41.4, 41.2, 41.5, 41.3, 41.6], dtype=np.float64)
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<SmaState>)> {
    if inputs.is_empty() {
    ...
    let result = match num_assets {
        2 => {
            ...
            Sma::indicator_by_assets::<2>(input_array, &options_array, optional_outputs.as_deref())
        }
        4 => {
            let input_array: &[&[&[f64]; INPUTS]; 4] = input_refs.as_slice().try_into().unwrap();
            Sma::indicator_by_assets::<4>(input_array, &options_array, optional_outputs.as_deref())
        }
        8 => {
            let input_array: &[&[&[f64]; INPUTS]; 8] = input_refs.as_slice().try_into().unwrap();
            Sma::indicator_by_assets::<8>(input_array, &options_array, optional_outputs.as_deref())
        }
        16 => {
            let input_array: &[&[&[f64]; INPUTS]; 16] = input_refs.as_slice().try_into().unwrap();
            Sma::indicator_by_assets::<16>(input_array, &options_array, optional_outputs.as_deref())
        }
    ...
    match result {
        Ok((results, states)) => {
            let sma_states: Vec<SmaState> = states
                ...
                .collect();
            Ok((crate::utils::simd_vecs_to_pyarrays(py, results), sma_states))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
/// Calculate SMA (Simple Moving Average) for a single asset with multiple options using SIMD
///
/// Parameters:
/// - inputs: List of numpy array [real]
/// - options: List of option arrays, where each array contains the period value
/// - optional_outputs: Optional list of booleans for additional outputs
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of SMA results for each option set
///   - states: Vector of SmaState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            Sma::indicator_by_options::<2>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        4 => {
            let opt_array: &[&[f64; OPTIONS]; 4] = option_refs.as_slice().try_into().unwrap();
            Sma::indicator_by_options::<4>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        8 => {
            let opt_array: &[&[f64; OPTIONS]; 8] = option_refs.as_slice().try_into().unwrap();
            Sma::indicator_by_options::<8>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        16 => {
            let opt_array: &[&[f64; OPTIONS]; 16] = option_refs.as_slice().try_into().unwrap();
            Sma::indicator_by_options::<16>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        _ => unreachable!("Already validated SIMD lane count"),
    };

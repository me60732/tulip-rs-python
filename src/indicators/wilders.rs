...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, WildersState)> {
    if inputs.len() != rust_wilders::INPUTS_WIDTH {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            rust_wilders::INPUTS_WIDTH,
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<WildersState>)> {
    if inputs.is_empty() {
    ...
    let input_refs: Vec<&[&[f64]; rust_wilders::INPUTS_WIDTH]> =
...
pub fn simd_by_options(
    ...
    let mut option_arrays: Vec<[f64; rust_wilders::OPTIONS_WIDTH]> =

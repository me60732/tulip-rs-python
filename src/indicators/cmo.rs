...
pub fn indicator(
    ...
    match rust_cmo::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((result, CmoState { inner: state })),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
        ))),
...
pub fn min_data_accuracy(options: Vec<f64>, decimals: usize) -> PyResult<usize> {
    Ok(rust_cmo::min_data_accuracy(&options, decimals))
}

#[pyfunction]
pub fn output_length(data_len: usize, options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_cmo::output_length(data_len, &options))
}
...
///
/// The Chande Momentum Oscillator (CMO) is a momentum indicator that measures the
/// momentum of a security by comparing the sum of recent gains to the sum of recent
/// losses over a given period. Values range from -100 to +100.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [close] arrays
/// - options: Vector containing [period] for the CMO calculation
/// - optional_outputs: Optional list of booleans for additional outputs (none available for CMO)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of CMO results for each asset (each asset returns one CMO line)
...
/// close1 = np.array([10.3, 10.6, 10.8, 10.7, 11.0, 10.9, 11.1, 10.8, 10.6, 10.9, 11.2, 11.0, 11.3, 11.1, 11.4, 11.2, 11.5, 11.3, 11.6, 11.4], dtype=np.float64)
///
...
///
/// # CMO options: [period]
/// options = [14.0]  # 14-period CMO
///
/// # Calculate CMO for all assets using SIMD
/// outputs, states = ti.indicators.cmo_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Vec<f64>>>, Vec<CmoState>)> {
    if inputs.is_empty() {
    ...
    match result {
        Ok((results, states)) => {
            let cmo_states: Vec<CmoState> = states
                .into_iter()
                .map(|state| CmoState { inner: state })
                .collect();
            Ok((results, cmo_states))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
pub fn simd_by_options(
    ...
    let input_arrays: [&[f64]; rust_cmo::INPUTS_WIDTH] = [
        inputs[0].as_slice()?
    ];

...
use tulip_rs::indicators::wad::{Indicator, IndicatorState, TIndicatorState, Wad, INPUTS, OPTIONS};
...
pub fn indicator(
    ...
    match Wad::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Wad::INFO)
}

#[pyfunction]
pub fn min_data(_options: Vec<f64>) -> PyResult<usize> {
    let options_array: [f64; OPTIONS] = [];
    Ok(Wad::min_data(&options_array))
}

/// Calculate WAD (Weighted Accumulation/Distribution) for multiple assets using SIMD operations
///
...
/// - options: Empty vector (WAD has no options)
/// - optional_outputs: Optional list of booleans for additional outputs (none available for WAD)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of WAD results for each asset (each asset returns one line)
...
/// ]
///
/// # WAD has no options - pass empty list
/// options = []
///
/// # Calculate WAD for all assets using SIMD
/// outputs, states = ti.indicators.wad_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    // Validate options (WAD has no options)
    if !options.is_empty() {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} options, got {}",
            OPTIONS,
    ...
    let result = match num_assets {
        2 => {
            ...
            Wad::indicator_by_assets::<2>(input_array, &options_array, optional_outputs.as_deref())
        }
        4 => {
            let input_array: &[&[&[f64]; INPUTS]; 4] = input_refs.as_slice().try_into().unwrap();
            Wad::indicator_by_assets::<4>(input_array, &options_array, optional_outputs.as_deref())
        }
        8 => {
            let input_array: &[&[&[f64]; INPUTS]; 8] = input_refs.as_slice().try_into().unwrap();
            Wad::indicator_by_assets::<8>(input_array, &options_array, optional_outputs.as_deref())
        }
        16 => {
            let input_array: &[&[&[f64]; INPUTS]; 16] = input_refs.as_slice().try_into().unwrap();
            Wad::indicator_by_assets::<16>(input_array, &options_array, optional_outputs.as_deref())
        }

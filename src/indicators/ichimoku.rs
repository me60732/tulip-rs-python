...
use tulip_rs::indicators::ichimoku::{
    Ichimoku, Indicator, IndicatorByOptions, IndicatorState, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, IchimokuState)> {
    if inputs.len() != INPUTS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            INPUTS,
    ...
    match Ichimoku::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
            crate::utils::vecs_to_pyarrays(py, result),
            IchimokuState { inner: state },
        )),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Ichimoku::INFO)
}
...
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(Ichimoku::min_data(&options_array))
}

/// Calculate Ichimoku Cloud for multiple assets using SIMD operations
///
...
/// identifies trend direction, and generates timing signals.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [high, low, close] arrays
/// - options: Vector containing [short_period, long_period] for theIchimoku calculation
/// - optional_outputs: Optional list of booleans for additional outputs (lagging_span)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Ichimoku results for each asset (each asset returns 5 lines)
///   - states: Vector of IchimokuState objects for continuing calculations
///
/// Input Structure:
...
///
/// # Data for 4 assets, 100 periods each (SIMD requires 2, 4, 8, or 16 assets)
/// high1 = np.array([...], dtype=np.float64)
/// low1 = np.array([...], dtype=np.float64)
/// close1 = np.array([...], dtype=np.float64)
///
...
/// ]
///
/// # Ichimoku options: [short_period, long_period]
/// options = [9.0, 26.0]  # Short period = 9, Long period = 26
///
/// # Calculate Ichimoku for all assets using SIMD
/// outputs, states = ti.indicators.ichimoku_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<IchimokuState>)> {
    if inputs.is_empty() {
    ...
    let result = match num_assets {
        2 => {
            ...
            Ichimoku::indicator_by_assets::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            ...
            Ichimoku::indicator_by_assets::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            Ichimoku::indicator_by_assets::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            Ichimoku::indicator_by_assets::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
    ...
    match result {
        Ok((results, states)) => {
            let ichimoku_states: Vec<IchimokuState> = states
                .into_iter()
                .map(|state| IchimokuState { inner: state })
                .collect();
            Ok((
...
/// Calculate Ichimoku Cloud for a single asset with multiple options using SIMD
///
/// Parameters:
...
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Ichimoku results for each option set
///   - states: Vector of IchimokuState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            Ichimoku::indicator_by_options::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            ...
            Ichimoku::indicator_by_options::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            Ichimoku::indicator_by_options::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            Ichimoku::indicator_by_options::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
...
/// Register the Ichimoku indicator module with Python
///
/// This function creates a Python submodule for the Ichimoku indicator and registers
...
pub fn register_ichimoku_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "ichimoku")?;
    ...
    submodule.add_class::<IchimokuState>()?;

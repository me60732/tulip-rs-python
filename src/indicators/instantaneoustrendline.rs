...
pub fn indicator(
    ...
    match InstantaneousTrendline::indicator(
        ...
        Ok((result, state)) => Ok((
            ...
            InstantaneoustrendlineState { inner: state },
        )),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
...
pub fn simd_by_assets(
    ...
    Vec<InstantaneoustrendlineState>,
)> {
    if inputs.is_empty() {
    ...
    match result {
        Ok((results, states)) => {
            let instantaneoustrendline_states: Vec<InstantaneoustrendlineState> = states
                .into_iter()
                .map(|state| InstantaneoustrendlineState { inner: state })
                .collect();
            Ok((
                crate::utils::simd_vecs_to_pyarrays(py, results),
                instantaneoustrendline_states,
            ))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
pub fn register_instantaneoustrendline_module(
    parent_module: &pyo3::Bound<'_, PyModule>,
) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "instantaneoustrendline")?;

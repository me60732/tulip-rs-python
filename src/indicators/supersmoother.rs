...
use tulip_rs::indicators::supersmoother::{
    Indicator, IndicatorByOptions, IndicatorState, SuperSmoother, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Py<PyArray1<f64>>>, SuperSmootherState)> {
    if inputs.len() != INPUTS {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} inputs, got {}",
            INPUTS,
    ...
    match SuperSmoother::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
            crate::utils::vecs_to_pyarrays(py, result),
            SuperSmootherState { inner: state },
        )),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, SuperSmoother::INFO)
}
...
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(SuperSmoother::min_data(&options_array))
}

/// Calculate Ehlers Super Smoother for multiple assets using SIMD operations
///
/// This function processes multiple assets simultaneously for improved performance
/// using SIMD (Single Instruction, Multiple Data) operations.
///
/// The Ehlers Super Smoother is a two-pole Butterworth-inspired IIR low-pass filter
/// designed to remove aliasing and high-frequency noise from sampled price data while
/// preserving cycle content below the cutoff frequency. Unlike a simple moving average,
/// it has zero lag at DC and a much sharper roll-off.
///
/// Parameters:
/// - inputs: Vector of asset inputs, where each asset contains [price] arrays
/// - options: Vector containing [period] for the filter
/// - optional_outputs: Optional list of booleans for additional outputs (none available)
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Super Smoother results for each asset
///   - states: Vector of SuperSmootherState objects for continuing calculations
///
/// Input Structure:
/// The inputs parameter should be structured as:
/// ```
/// inputs = [
///     [price_asset1],  # Asset 1
///     [price_asset2],  # Asset 2
///     # ... more assets
/// ]
/// ```
///
...
/// price1 = np.array([...], dtype=np.float64)
/// price2 = np.array([...], dtype=np.float64)
/// price3 = np.array([...], dtype=np.float64)
/// price4 = np.array([...], dtype=np.float64)
///
...
/// ]
///
/// # Super Smoother options: [period]
/// options = [10.0]  # Filter period = 10
///
/// # Calculate Super Smoother for all assets using SIMD
/// outputs, states = ti.indicators.supersmoother_simd_by_assets(inputs, options, None)
/// ```
///
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Py<PyArray1<f64>>>>, Vec<SuperSmootherState>)> {
    if inputs.is_empty() {
    ...
    let result = match num_assets {
        2 => {
            ...
            SuperSmoother::indicator_by_assets::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            ...
            SuperSmoother::indicator_by_assets::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            SuperSmoother::indicator_by_assets::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            SuperSmoother::indicator_by_assets::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
    ...
    match result {
        Ok((results, states)) => {
            let supersmoother_states: Vec<SuperSmootherState> = states
                .into_iter()
                .map(|state| SuperSmootherState { inner: state })
                .collect();
            Ok((
                crate::utils::simd_vecs_to_pyarrays(py, results),
                supersmoother_states,
            ))
        }
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
/// Calculate Ehlers Super Smoother for a single asset with multiple options using SIMD
///
/// Parameters:
/// - inputs: List of numpy arrays [price]
...
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Super Smoother results for each option set
///   - states: Vector of SuperSmootherState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            SuperSmoother::indicator_by_options::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            ...
            SuperSmoother::indicator_by_options::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            ...
            SuperSmoother::indicator_by_options::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            ...
            SuperSmoother::indicator_by_options::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
...
/// Register the SuperSmoother indicator module with Python
///
/// This function creates a Python submodule for the SuperSmoother indicator and registers
...
pub fn register_supersmoother_module(
    parent_module: &pyo3::Bound<'_, PyModule>,
) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "supersmoother")?;
    ...
    submodule.add_class::<SuperSmootherState>()?;

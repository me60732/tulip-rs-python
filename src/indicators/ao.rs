...
impl AoState {
    ...
    fn batch_indicator(
        ...
        if inputs.len() != rust_ao::INPUTS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                "Expected {} inputs, got {}",
                rust_ao::INPUTS_WIDTH,
                inputs.len()
            )));
        }

        let input_arrays: [&[f64]; rust_ao::INPUTS_WIDTH] =
...
pub fn indicator(
    ...
    if options.len() != rust_ao::OPTIONS_WIDTH {
        return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
            "Expected {} options, got {}",
            rust_ao::OPTIONS_WIDTH,
            options.len()
        )));
    }
    ...
    let options_array: [f64; rust_ao::OPTIONS_WIDTH] = [];

    match rust_ao::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((result, AoState { inner: state })),
        Err(e) => Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
            "Indicator calculation failed: {:?}",
            e
        ))),
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, rust_ao::INFO)
}

#[pyfunction]
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_ao::min_data(&options))
}

#[pyfunction]
pub fn min_data_accuracy(options: Vec<f64>, decimals: usize) -> PyResult<usize> {
    Ok(rust_ao::min_data_accuracy(&options, decimals))
}

#[pyfunction]
pub fn output_length(data_len: usize, options: Vec<f64>) -> PyResult<usize> {
    Ok(rust_ao::output_length(data_len, &options))
}
...
pub fn simd_by_assets(
    ...
    options: Vec<f64>,
    optional_outputs: Option<Vec<bool>>,
) -> PyResult<(Vec<Vec<Vec<f64>>>, Vec<AoState>)> {
    if inputs.is_empty() {
    ...
    for (asset_idx, asset_inputs) in inputs.iter().enumerate() {
        if asset_inputs.len() != rust_ao::INPUTS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let mut asset_input_arrays: Vec<[&[f64]; rust_ao::INPUTS_WIDTH]> =
        Vec::with_capacity(num_assets);

    for asset_inputs in &inputs {
        let input_array: [&[f64]; rust_ao::INPUTS_WIDTH] = [
    ...
    let input_refs: Vec<&[&[f64]; rust_ao::INPUTS_WIDTH]> = asset_input_arrays.iter().collect();
    ...
    let result = match num_assets {
        2 => {
            let input_array: &[&[&[f64]; rust_ao::INPUTS_WIDTH]; 2] =
                input_refs.as_slice().try_into().unwrap();
            rust_ao::by_assets::indicator::<2>(
                input_array,
                &options_array,
        ...
        4 => {
            let input_array: &[&[&[f64]; rust_ao::INPUTS_WIDTH]; 4] =
                input_refs.as_slice().try_into().unwrap();
            rust_ao::by_assets::indicator::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let input_array: &[&[&[f64]; rust_ao::INPUTS_WIDTH]; 8] =
                input_refs.as_slice().try_into().unwrap();
            rust_ao::by_assets::indicator::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let input_array: &[&[&[f64]; rust_ao::INPUTS_WIDTH]; 16] =
                input_refs.as_slice().try_into().unwrap();
            rust_ao::by_assets::indicator::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )

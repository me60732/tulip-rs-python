...
use tulip_rs::indicators::vortex::{
    Indicator, IndicatorByOptions, IndicatorState, Vortex, INPUTS, OPTIONS,
};
...
pub fn indicator(
    ...
    match Vortex::indicator(&input_arrays, &options_array, optional_outputs.as_deref()) {
        Ok((result, state)) => Ok((
...
pub fn info(py: Python<'_>) -> PyResult<Bound<'_, pyo3::types::PyDict>> {
    crate::utils::info_to_pydict(py, Vortex::INFO)
}
...
pub fn min_data(options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(Vortex::min_data(&options_array))
}
...
/// ]
///
/// # Vortex options: [period]
/// options = [14.0]  # Period = 14
///
/// # Calculate Vortex for all assets using SIMD
/// outputs, states = ti.indicators.vortex_simd_by_assets(inputs, options, None)
/// ```
///
...
/// Calculate Vortex Indicator for a single asset with multiple options using SIMD
///
/// Parameters:
...
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of Vortex results for each option set
///   - states: Vector of VortexState objects for continuing calculations
#[pyfunction]
#[pyo3(signature = (inputs, options, optional_outputs=None))]
pub fn simd_by_options(
    ...
    let result = match num_options {
        2 => {
            ...
            Vortex::indicator_by_options::<2>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        4 => {
            let opt_array: &[&[f64; OPTIONS]; 4] = option_refs.as_slice().try_into().unwrap();
            Vortex::indicator_by_options::<4>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        8 => {
            let opt_array: &[&[f64; OPTIONS]; 8] = option_refs.as_slice().try_into().unwrap();
            Vortex::indicator_by_options::<8>(&input_arrays, opt_array, optional_outputs.as_deref())
        }
        16 => {
            let opt_array: &[&[f64; OPTIONS]; 16] = option_refs.as_slice().try_into().unwrap();
            Vortex::indicator_by_options::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
...
///
/// This function creates a Python submodule for the Vortex indicator and registers

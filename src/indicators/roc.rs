    ...
    fn batch_indicator(
        ...
        if inputs.len() != rust_roc::INPUTS_WIDTH {
            return Err(pyo3::exceptions::PyValueError::new_err(format!(
                ...
                rust_roc::INPUTS_WIDTH,
                inputs.len()
            )));
        }

        // Direct extraction for single input (real)
        let inputs_array: [&[f64]; rust_roc::INPUTS_WIDTH] = [inputs[0].as_slice()?];
...
pub fn simd_by_options(
    ...
    for (opt_idx, opt) in options.iter().enumerate() {
        if opt.len() != rust_roc::OPTIONS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let option_refs: Vec<&[f64; rust_roc::OPTIONS_WIDTH]> = option_arrays.iter().collect();

    // Call the SIMD by options function with proper const generic
    let result = match num_options {
        2 => {
            let opt_array: &[&[f64; rust_roc::OPTIONS_WIDTH]; 2] =
                option_refs.as_slice().try_into().unwrap();
            rust_roc::by_options::indicator::<2>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            let opt_array: &[&[f64; rust_roc::OPTIONS_WIDTH]; 4] =
                option_refs.as_slice().try_into().unwrap();
            rust_roc::by_options::indicator::<4>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let opt_array: &[&[f64; rust_roc::OPTIONS_WIDTH]; 8] =
                option_refs.as_slice().try_into().unwrap();
            rust_roc::by_options::indicator::<8>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let opt_array: &[&[f64; rust_roc::OPTIONS_WIDTH]; 16] =
                option_refs.as_slice().try_into().unwrap();
            rust_roc::by_options::indicator::<16>(
                &input_arrays,
                opt_array,
                optional_outputs.as_deref(),
            )
    ...
    match result {
        Ok((results, states)) => {
            let roc_states: Vec<RocState> = states
                .into_iter()
...
/// Register the ROC indicator module with Python
///
/// This function creates a Python submodule for the ROC indicator and registers
...
pub fn register_roc_module(parent_module: &pyo3::Bound<'_, PyModule>) -> pyo3::PyResult<()> {
    let submodule = PyModule::new(parent_module.py(), "roc")?;
...
/// Calculate ROC (Rate of Change) for multiple assets using SIMD operations
///
/// This function processes multiple assets simultaneously for improved performance
/// using SIMD (Single Instruction, Multiple Data) operations.
///
/// The Rate of Change (ROC) is a momentum indicator that measures the percentage
/// change in price from one period to the next. It's calculated as:
...
/// - options: Vector containing [period] for the ROC calculation
/// - optional_outputs: Optional list of booleans for additional outputs [mom]
///
/// Returns:
/// - Tuple of (outputs, states) where:
///   - outputs: Vector of ROC results for each asset (each asset returns one ROC line + optional mom)
///   - states: Vector of RocState objects for continuing calculations
///
/// Input Structure:
...
///
/// # ROC options: [period]
/// options = [5]  # 5-period ROC
///
/// # Calculate ROC for all assets using SIMD
/// outputs, states = ti.indicators.roc_simd_by_assets(inputs, options, None)
///
/// # outputs[0] contains ROC values for asset 1
/// # outputs[1] contains ROC values for asset 2
/// # outputs[2] contains ROC values for asset 3
/// # outputs[3] contains ROC values for asset 4
...
pub fn simd_by_assets(
    ...
    for (asset_idx, asset_inputs) in inputs.iter().enumerate() {
        if asset_inputs.len() != rust_roc::INPUTS_WIDTH {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
    ...
    let mut asset_input_arrays: Vec<[&[f64]; rust_roc::INPUTS_WIDTH]> =
        Vec::with_capacity(num_assets);

    for asset_inputs in &inputs {
        let input_array: [&[f64]; rust_roc::INPUTS_WIDTH] = [
    ...
    let input_refs: Vec<&[&[f64]; rust_roc::INPUTS_WIDTH]> = asset_input_arrays.iter().collect();
    ...
    let result = match num_assets {
        2 => {
            let input_array: &[&[&[f64]; rust_roc::INPUTS_WIDTH]; 2] =
                input_refs.as_slice().try_into().unwrap();
            rust_roc::by_assets::indicator::<2>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        4 => {
            let input_array: &[&[&[f64]; rust_roc::INPUTS_WIDTH]; 4] =
                input_refs.as_slice().try_into().unwrap();
            rust_roc::by_assets::indicator::<4>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        8 => {
            let input_array: &[&[&[f64]; rust_roc::INPUTS_WIDTH]; 8] =
                input_refs.as_slice().try_into().unwrap();
            rust_roc::by_assets::indicator::<8>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
        ...
        16 => {
            let input_array: &[&[&[f64]; rust_roc::INPUTS_WIDTH]; 16] =
                input_refs.as_slice().try_into().unwrap();
            rust_roc::by_assets::indicator::<16>(
                input_array,
                &options_array,
                optional_outputs.as_deref(),
            )
...
pub fn min_data_accuracy(options: Vec<f64>, decimals: usize) -> PyResult<usize> {
    ...
    Ok(rust_roc::min_data_accuracy(&options, decimals))
}
...
pub fn output_length(data_len: usize, options: Vec<f64>) -> PyResult<usize> {
    ...
    Ok(rust_roc::output_length(data_len, &options))
}
